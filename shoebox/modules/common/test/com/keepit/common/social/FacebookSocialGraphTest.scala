  ...
  "FacebookSocialGraph" should {
    ...
    "fetch from facebook" in {
      running(new EmptyApplication()) {
        ...
        val json = io.Source.fromFile(new File("modules/common/test/com/keepit/common/social/facebook_graph_eishay_super_min.json")).mkString
    ...
    "fetch from facebook using jennifer_hirsch" in {
      running(new EmptyApplication()) {
        //val httpClient = HttpClientImpl(timeout = 1, timeoutUnit = TimeUnit.MINUTES)
        val data = io.Source.fromFile(new File("modules/common/test/com/keepit/common/social/jennifer_hirsch.min.json")).mkString
