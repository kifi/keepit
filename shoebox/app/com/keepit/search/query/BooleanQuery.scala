...
object BooleanQueryWithPercentMatch {
  def apply(clauses: JList[BooleanClause], percentMatch: Float, disableCoord: Boolean) = {
    val query = new BooleanQueryWithPercentMatch(percentMatch, disableCoord)
    clauses.foreach{ clause => query.add(clause) }
    query
  }
}

class BooleanQueryWithPercentMatch(val percentMatch: Float, val disableCoord: Boolean = false) extends LBooleanQuery(disableCoord) {
  ...
  override def rewrite(reader: IndexReader) = {
    ...
    val rewrittenQuery = new BooleanQueryWithPercentMatch(percentMatch, disableCoord) // recursively rewrite
  ...
  override def clone(): Object = {
    val clone = new BooleanQueryWithPercentMatch(percentMatch, disableCoord)
