...
trait InvitationRepo extends Repo[Invitation] {
...
@Singleton
class InvitationRepoImpl @Inject() (val db: DataBaseComponent, val clock: Clock) extends DbRepo[Invitation] with InvitationRepo {
  import FortyTwoTypeMappers._
  import scala.slick.lifted.Query
  import db.Driver.Implicit._
  import DBSession._

  override lazy val table = new RepoTable[Invitation](db, "invitation") {
    ...
    def * = id.? ~ createdAt ~ updatedAt ~ senderUserId ~ recipientSocialUserId ~ state <> (Invitation, Invitation.unapply _)
  }

  def getByUser(userId: Id[User])(implicit session: RSession): Seq[Invitation] =
    (for(b <- table if b.senderUserId === userId) yield b).list
