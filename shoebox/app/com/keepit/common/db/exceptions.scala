...
class ElementWithExternalIdNotFoundException[T](m: Manifest[T], id: ExternalId[T])
  extends NoSuchElementException("can't find %s with external id %s".format(m, id))

class ElementWithInternalIdNotFoundException[T](m: Manifest[T], id: Id[T])
  extends NoSuchElementException("can't find %s with internal id %s".format(m, id))

class ElementWithInternalIdsNotFoundException[T,U,V](c: Class[T], m1: Manifest[U], id1: Id[U], m2: Manifest[V], id2: Id[V])
  extends NoSuchElementException("can't find %s with internal %s id %s and %s id %s".format(c.getName(), m1, id1, m2, id2))
