...
class SearchStatisticsSerializerTest extends Specification {
  "SearchStatistics Serializer" should {
    "correctly serialize" in {
      ...
      val uriInfo = UriInfo(uriId = Id[NormalizedURI](2L), textScore = 1.0f, normalizedTextScore = 2.0f, bookmarkScore = 3.0f,
        recencyScore = 4.0f, clickBoost = 5.0f, isMyBookmark = false, isPrivate = false,
        friendsKeepsCount = 1, totalCounts = 7, incorrectlyRanked = false)
      ...
      val searchStat = SearchStatistics(qInfo, uriInfo, sInfo, lscore)
