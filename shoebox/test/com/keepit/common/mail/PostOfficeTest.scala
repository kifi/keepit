    ...
    "persist and load email" in {
      withDB(FakeMailModule()) { implicit injector =>
        ...
        val mail2 = db.readWrite { implicit s =>
          inject[LocalPostOffice].sendMail(ElectronicMail(from = EmailAddresses.ENG, to = List(EmailAddresses.TEAM), subject = "foo 2", htmlBody = "some body in html 2", category = PostOffice.Categories.HEALTHCHECK))
        }
        outbox.size === 2
