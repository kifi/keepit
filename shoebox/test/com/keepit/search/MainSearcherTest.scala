      ...
      running(new EmptyApplication()) {
        ...
        val bookmarkMap = CX.withConnection { implicit c =>
          uris.foldLeft(Map.empty[Id[NormalizedURI], Bookmark]){ (m, uri) =>
            ...
            m + (uriId -> Bookmark(createdAt = createdAt, title = uri.title.get, metadata = Some(NormalizedURIMetadata(uri.url, "", uri.id.get)),  uriId = uriId, userId = userId, source = source).save)
          }
        }
