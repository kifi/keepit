...
this.tagbox = (function (win) {
	...
	function onLiveChange(e) {
		$suggest.empty();
		filterAndRender(e.value);
	}

	function filterTags(list, val) {
		return win.fuzzy.filter(val, list, {
			pre: '<b>',
			post: '</b>',
	...
	function filterAndRender(val) {
		...
		if (hasMatch) {
			matches.forEach(renderSuggestion);
		}
		$tagbox.toggleClass('suggested', hasMatch);
	}
	...
	function activateScroll(selector) {
		...
		win.log('here1')();
		$(win).on('resize' + selector, function () {
			win.log('here2')();
	...
	return {
		show: function ($slider) {
			if (!$tagbox) {
				win.render('html/keeper/tagbox', {}, function (html) {
					...
					$input = $tagbox.find('input.kifi-tagbox-input');
					checkInputValue = createValueTimeout($input);
					$input.on('focus', onFocus);
					$input.on('blur', onBlur);
					$input.on('keydown change input paste', onChange);
					$input.on('livechange', onLiveChange);

					$suggest = $tagbox.find('.kifi-tagbox-suggest');
					$tagbox.find('.kifi-tagbox-close').click(this, function (e) {
						e.data.remove();
					});
		...
		remove: function () {
			if ($tagbox) {
				...
				$suggest.remove();
				$tagbox.remove();
				$tagbox = $input = $suggest = checkInputValue = null;
			}
		}
	};
