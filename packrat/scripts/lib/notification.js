...
var KifiNotification = {
	...
	add: function(params){
		...
		var title = params.title, 
			...
			popupClass = params.class_name || '',
			position = params.position || '',
			closeTime = params.time || this.time,
			clickAction = params.click || function() {},
		...
		var image_str = (image != '') ? '<img src="' + image + '" class="kifi-notify-image" />' : '',
			class_name = (image != '') ? 'kifi-notify-with-image' : 'kifi-notify-without-image';
      ...
      'link': link
			...
			item.fadeIn(KifiNotification.fadeInSpeed, function(){
	...
	fadeItem: function(e, unique_id, params, unbind_events){
		...
		if(fade){
			e.animate({ opacity: 0 }, fadeOutSpeed, function(){
