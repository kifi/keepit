...
object DevGlobal extends FortyTwoGlobal(Dev) with ShoeboxServices with SearchServices {
  override val module = Modules.`override`(ABookDevModule()).`with`(Modules.`override`(HeimdalDevModule()).`with`(Modules.`override`(ElizaDevModule()).`with`(Modules.`override`(SearchDevModule()).`with`(ShoeboxDevModule()))))
