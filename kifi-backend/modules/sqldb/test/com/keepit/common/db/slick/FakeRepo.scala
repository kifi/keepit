...
class FakeRepo[M <: Model[M]] extends Repo[M] {
  ...
  def getNoCache(id: Id[M])(implicit session: RSession): M = db(id)
  def aTonOfRecords()(implicit session: RSession): Seq[M] = db.values.toSeq
  ...
  def pageAscending(page: Int = 0, size: Int = 20, excludeStates: Set[State[M]] = Set.empty[State[M]])(implicit session: RSession): Seq[M] = db.values.drop(page * size).take(size).toSeq
