...
class PaidPlanRepoImpl @Inject() (
  ...
  implicit val featureSetTypeMapper = MappedColumnType.base[Set[Feature[_]], String](
    { obj => Json.stringify(Json.toJson(obj)) },
    { str => Json.parse(str).as[Set[Feature[_]]] }
  )
  ...
  class PaidPlanTable(tag: Tag) extends RepoTable[PaidPlan](db, tag, "paid_plan") {
    ...
    def editableFeatures = column[Set[Feature[_]]]("editable_features", O.NotNull)
