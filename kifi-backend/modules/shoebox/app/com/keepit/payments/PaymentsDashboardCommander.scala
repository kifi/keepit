...
class PaymentsDashboardCommander(
    db: Database,
  ...
  def generateDashboard(): AdminPaymentsDashboard = db.readOnlyMaster { implicit session =>
    ...
    val oldPlanEnrollments = {
      ...
      val m_planByAccount = mutable.Map(paidAccountRepo.all.map(a => a.id.get -> a.planId): _*)
      ...
      accountEventRepo.adminGetByKindSince(AccountEventKind.all, now.minusWeeks(1)).map(e => (e.accountId, e.action)).foreach {
        case (accountId, AccountEventAction.PlanChanged(oldPlan, newPlan, _)) =>
          m_planByAccount(accountId) = oldPlan
        case (accountId, AccountEventAction.UserJoinedOrganization(_, _)) =>
          m_usersByAccount(accountId) = m_usersByAccount(accountId) - 1
      ...
      plans.map { p =>
        p -> PlanEnrollment(
          numAccounts = m_planByAccount.collect { case (accountId, planId) if planId == p.id.get => 1 }.sum,
          numActiveUsers = m_usersByAccount.collect { case (accountId, numUsers) if m_planByAccount(accountId) == p.id.get => numUsers }.sum
        )
      }.toMap
    }
