...
class ProtoOrganizationMembershipRepoImpl @Inject() (val db: DataBaseComponent, val clock: Clock) extends ProtoOrganizationMembershipRepo with DbRepo[ProtoOrganizationMembership] with Logging {
  ...
  class ProtoOrganizationMembershipTable(tag: Tag) extends RepoTable[ProtoOrganizationMembership](db, tag, "proto_organization_invite") {
    ...
    def * = (id.?, createdAt, updatedAt, state, protoOrgId, inviterId, userId, emailAddress) <> ((ProtoOrganizationMembership.apply _).tupled, ProtoOrganizationMembership.unapply)
  }
