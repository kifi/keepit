...
class LibraryInviteRepoImpl @Inject() (
  ...
  def getWithLibraryIdsAndUserId(libraryIds: Set[Id[Library]], userId: Id[User], includeSet: Set[State[LibraryInvite]] = Set(LibraryInviteStates.ACTIVE))(implicit session: RSession): Map[Id[Library], Seq[LibraryInvite]] = {
    val foundInvites = rows.filter(b => b.libraryId.inSet(libraryIds) && b.userId === userId && b.state.inSet(includeSet)).sortBy(_.createdAt).list.groupBy(_.libraryId)
    libraryIds.map { libId => libId -> foundInvites.getOrElse(libId, Seq.empty) }.toMap
  }
