...
trait KeepRepo extends Repo[Keep] with ExternalIdColumnFunction[Keep] with SeqNumberFunction[Keep] {
  ...
  def getRecentKeepsFromFollowedLibraries(userId: Id[User], limit: Int)(implicit session: RSession): Seq[Keep]
...
class KeepRepoImpl @Inject() (
  ...
  def getRecentKeepsFromFollowedLibraries(userId: Id[User], limit: Int)(implicit session: RSession): Seq[Keep] = {
    import com.keepit.common.db.slick.StaticQueryFixed.interpolation
    val recentLibraryUpdates = sql"""SELECT #$bookmarkColumnOrder FROM bookmark bm WHERE library_id IN (SELECT library_id FROM library_membership WHERE user_id=$userId AND state='active') AND state='active' AND user_id!=$userId ORDER BY kept_at DESC LIMIT $limit;""".as[Keep].list
    recentLibraryUpdates
  }
