...
trait KeepRepo extends Repo[Keep] with ExternalIdColumnFunction[Keep] with SeqNumberFunction[Keep] {
  ...
  def getRecentKeepsFromFollowedLibraries(userId: Id[User], num: Int, beforeIdOpt: Option[ExternalId[Keep]], afterIdOpt: Option[ExternalId[Keep]])(implicit session: RSession): Seq[Keep]
...
class KeepRepoImpl @Inject() (
  ...
  def getByLibraryWithoutOrgId(libraryId: Id[Library], orgIdOpt: Option[Id[Organization]], offset: Offset, limit: Limit)(implicit session: RSession): Seq[Keep] = {
    import com.keepit.common.db.slick.StaticQueryFixed.interpolation
    (orgIdOpt match {
      case None => sql"select * from bookmark where library_id = $libraryId and organization_id is not null limit ${limit.value} offset ${offset.value}"
      case _ => sql"select * from bookmark where library_id = $libraryId and (organization_id != ${orgIdOpt.get} or organization_id is null) limit ${limit.value} offset ${offset.value}"
    }).as[Keep].list
  }
  ...
  def getRecentKeepsFromFollowedLibraries(userId: Id[User], num: Int, beforeIdOpt: Option[ExternalId[Keep]], afterIdOpt: Option[ExternalId[Keep]])(implicit session: RSession): Seq[Keep] = {
    ...
    (beforeIdOpt.flatMap(getOpt), afterIdOpt.flatMap(getOpt)) match {
      case (Some(before), _) =>
        sql"""SELECT #$bookmarkColumnOrder FROM bookmark bm WHERE library_id IN (SELECT library_id FROM library_membership WHERE user_id=$userId AND state='active') AND state='active' AND user_id!=$userId AND kept_at <= ${before.keptAt} AND id < ${before.id.get} ORDER BY kept_at DESC LIMIT $num;""".as[Keep].list
      case (None, Some(after)) =>
        ...
        sql"""SELECT #$bookmarkColumnOrder FROM bookmark bm WHERE library_id IN (SELECT library_id FROM library_membership WHERE user_id=$userId AND state='active') AND state='active' AND user_id!=$userId AND kept_at > ${after.keptAt} ORDER BY kept_at DESC LIMIT $num;""".as[Keep].list
      case (None, None) =>
