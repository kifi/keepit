...
import com.keepit.model.UserNotificationStates.{INACTIVE, UNDELIVERED, DELIVERED, VISITED, SUBSUMED}
...
trait UserNotificationRepo extends Repo[UserNotification] with ExternalIdColumnFunction[UserNotification]  {
  ...
  def getLatestFor(userId: Id[User], howMany: Int = 10, excludeStates: Set[State[UserNotification]] = Set(INACTIVE, SUBSUMED))(implicit session: RSession): Seq[UserNotification]
  def getCreatedAfter(userId: Id[User], time: DateTime, excludeStates: Set[State[UserNotification]] = Set(INACTIVE, SUBSUMED))(implicit session: RSession): Seq[UserNotification]
  def getCreatedBefore(userId: Id[User], time: DateTime, howMany: Int, excludeStates: Set[State[UserNotification]] = Set(INACTIVE, SUBSUMED))(implicit session: RSession): Seq[UserNotification]
  def getWithCommentId(userId: Id[User], commentId: Id[Comment], excludeStates: Set[State[UserNotification]] = Set(INACTIVE, SUBSUMED))(implicit session: RSession): Option[UserNotification]
  def getWithCommentIds(userId: Id[User], commentIds: Traversable[Id[Comment]], excludeStates: Set[State[UserNotification]] = Set(INACTIVE, SUBSUMED))(implicit session: RSession): Seq[UserNotification]
  ...
  def markVisited(userId: Id[User], noticeExternalId: ExternalId[UserNotification], excludeStates: Set[State[UserNotification]] = Set(INACTIVE, SUBSUMED, VISITED))(implicit session: RWSession): Int
  def markCommentVisited(userId: Id[User], commentIds: Traversable[Id[Comment]], excludeStates: Set[State[UserNotification]] = Set(INACTIVE, SUBSUMED, VISITED))(implicit session: RWSession): Int
}
...
class UserNotificationRepoImpl @Inject() (
  ...
  def allActive(category: UserNotificationCategory)(implicit session: RSession): Seq[UserNotification] =
    (for (b <- table if b.state =!= SUBSUMED && b.state =!= INACTIVE && b.category === category) yield b).list

  def allUndelivered(before: DateTime)(implicit session: RSession): Seq[UserNotification] =
    (for (b <- table if b.state === UNDELIVERED && b.createdAt < before) yield b).list
  ...
  def getUnvisitedCount(userId: Id[User])(implicit session: RSession): Int =
    Query((for (n <- table if n.userId === userId && n.state.inSet(Set(UNDELIVERED, DELIVERED))) yield n).length).first
  ...
  def markVisited(userId: Id[User], noticeExternalId: ExternalId[UserNotification], excludeStates: Set[State[UserNotification]] = Set(INACTIVE, SUBSUMED, VISITED))(implicit session: RWSession): Int =
    (for(n <- table if n.userId === userId && !n.state.inSet(excludeStates) && n.externalId === noticeExternalId) yield n.state ~ n.updatedAt)
      .update((VISITED, currentDateTime))
