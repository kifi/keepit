...
class CollectionRepoImpl @Inject() (
  ...
  def getByUserSortedByNumKeeps(userId: Id[User], page: Int, size: Int)(implicit session: RSession): Seq[(CollectionSummary, Int)] = {
    import StaticQuery.interpolation
    import scala.collection.JavaConversions._
    val query = sql"select c.id, c.external_id, c.name, count(kc.id), b as keep_count from collection c, keep_to_collection kc, bookmark b where kc.collection_id=c.id and kc.bookmark_id=b.id and user_id=${userId} and b.state=${KeepStates.ACTIVE} kc.state=${KeepToCollectionStates.ACTIVE} and c.state=${CollectionStates.ACTIVE} group by c.id order by keep_count desc limit ${size} offset ${page * size}"
