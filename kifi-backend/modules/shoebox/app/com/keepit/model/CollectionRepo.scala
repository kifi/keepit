...
class CollectionRepoImpl @Inject() (
  ...
  def getDuplicateCollections()(implicit session: RSession): Map[(Id[User], Hashtag), Set[Collection]] = {
    import StaticQuery.interpolation
    val query = sql"select user_id, name from (select c.user_id, c.name, count(c.id) as collection_count from collection group by c.user_id, c.name) as collection_by_user_and_tag where collection_count > 1"
