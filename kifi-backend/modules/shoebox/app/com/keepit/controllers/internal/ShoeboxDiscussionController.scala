...
import com.keepit.commanders.{ KeepCommander, RawBookmarkRepresentation, SocialShare }
...
class ShoeboxDiscussionController @Inject() (
  ...
  def internKeep() = Action(parse.tolerantJson) { request =>
    ...
    val (keep, _) = keepCommander.keepOne(rawBookmark, owner, libraries.head, None, KeepSource.keeper, SocialShare.empty)
