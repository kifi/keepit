        ...
        keepers.foreach { extId =>
          ...
          keepRepo.getByUriAndUser(uriId, keeperId) match {
            case None =>
              ...
              // move on
            case Some(keep) =>
              // todo: check for abuse
              val kc = KeepClick(searchUUID = randomUUID, numKeepers = keepers.length, keeperId = keeperId, keepId = keep.id.get, uriId = keep.uriId)
              log.info(s"[clickAttribution($clicker, $uriId, ${keepers.mkString(",")})] saving $kc")
              keepClicksRepo.save(kc)
          }
        }
      }
    }
