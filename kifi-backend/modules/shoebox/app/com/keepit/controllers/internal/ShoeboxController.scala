...
class ShoeboxController @Inject() (
  ...
  def internKeep() = Action(parse.tolerantJson) { request =>
    ...
    val internResponse = keepInterner.internRawBookmarksWithStatus(Seq(rawBookmark), Some(input.creator), libraryOpt = None, Some(input.users), source = KeepSource.discussion)
