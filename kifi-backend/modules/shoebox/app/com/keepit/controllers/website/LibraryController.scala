...
class LibraryController @Inject() (
  ...
  def getLibraryById(pubId: PublicId[Library], universalToken: Option[String] = None, authToken: Option[String] = None, passcode: Option[HashedPassPhrase] = None) = JsonAction.authenticatedAsync { request =>
    val idTry = Library.decodePublicId(pubId)
    idTry match {
      case Failure(ex) =>
      ...
      case Success(id) =>
        ...
        if (libraryCommander.canViewLibrary(Some(request.userId), lib, universalToken, authToken, passcode)) {
  ...
  def getLibraryByPath(userStr: String, slugStr: String, universalToken: Option[String] = None, authToken: Option[String] = None, passcode: Option[HashedPassPhrase] = None) = JsonAction.authenticatedAsync { request =>
  ...
  def getKeeps(pubId: PublicId[Library], count: Int, offset: Int, universalToken: Option[String] = None, authToken: Option[String] = None, passcode: Option[HashedPassPhrase] = None) = JsonAction.authenticatedAsync { request =>
    val idTry = Library.decodePublicId(pubId)
    idTry match {
      case Failure(ex) =>
        Future.successful(BadRequest(Json.obj("error" -> "invalid_id")))
  ...
  def getCollaborators(pubId: PublicId[Library], count: Int, offset: Int, universalToken: Option[String] = None, authToken: Option[String] = None, passcode: Option[HashedPassPhrase] = None) = JsonAction.authenticated { request =>
    val idTry = Library.decodePublicId(pubId)
    idTry match {
      case Failure(ex) =>
        BadRequest(Json.obj("error" -> "invalid_id"))
