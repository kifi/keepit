...
class Scraper @Inject() (
  ...
  private def isFishy(movedUri: NormalizedURI)(implicit session: RSession): Boolean =
    movedUri.restriction == Some(Restriction.http(301)) || bookmarkRepo.latestBookmark(movedUri.id.get).map(_.updatedAt.isAfter(currentDateTime.minusHours(1))).getOrElse(false)
}
