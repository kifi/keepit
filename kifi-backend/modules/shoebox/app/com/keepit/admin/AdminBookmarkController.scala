  ...
  def bookmarksView(page: Int = 0) = AdminHtmlAction.authenticatedAsync { implicit request =>
    ...
    def bookmarksInfos() = {
      future { timing(s"load $PAGE_SIZE bookmarks") { db.readOnlyReplica { implicit s => keepRepo.page(page, PAGE_SIZE, false, Set(KeepStates.INACTIVE)) } } } flatMap { bookmarks =>
        val usersFuture = future { timing("load user") { db.readOnlyReplica { implicit s =>
          bookmarks map (_.userId) map { id =>
            userMap.getOrElseUpdate(id, userRepo.get(id))
          }
        }}}
        val urisFuture = future { timing("load uris") { db.readOnlyReplica { implicit s =>
          bookmarks map (_.uriId) map uriRepo.get
        }}}
        val scrapesFuture = future { timing("load scrape info") { db.readOnlyReplica { implicit s =>
    ...
    val bookmarkTodayAllCountsFuture = future { timing("load bookmarks counts from today") { db.readOnlyReplica { implicit s =>
      keepRepo.getAllCountsByTimeAndSource(clock.now().minusDays(1), clock.now())
    }}}
    val privateKeeperKeepCountFuture = future { timing("load private keeper counts from today") { db.readOnlyReplica { implicit s =>
