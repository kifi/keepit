...
class AdminUserController @Inject() (
  ...
  def userView(userId: Id[User]) = AdminHtmlAction { implicit request =>
    ...
    val contacts:Seq[ABookRawInfo] = Await.result(abookClient.getAllContactsRawInfo(userId), 5 seconds)
    val abooks:Seq[ABook] = Await.result(abookClient.getABooks(userId), 5 seconds)

    Ok(html.admin.user(user, bookmarks.size, experiments, filteredBookmarks, socialUsers, socialConnections,
      fortyTwoConnections, kifiInstallations, historyUpdateCount, bookmarkSearch, allowedInvites, emails, abooks, contacts,
