...
class TypeaheadAdminController @Inject() (
  ...
  def search(userId:Id[User], query:String, limit:Int, pictureUrl:Boolean, filterJoinedUsers:Boolean, addNFUsers:Boolean) = AdminHtmlAction.authenticatedAsync { request =>
    ...
    Ok(
        "<table border=1><tr><td>label</td><td>networkType</td><td>score</td><td>status</td><td>value</td><td>image</td><td>extId</td></tr>" +
        res.map(c => s"<tr><td>${c.label}</td><td>${c.networkType}</td><td>${c.score}</td><td>${c.status}</td><td>${c.value}</td><td>${c.image}</td><td>${c.externalId}</td></tr>").mkString("") +
        "</table>"
      )
    }
  }
