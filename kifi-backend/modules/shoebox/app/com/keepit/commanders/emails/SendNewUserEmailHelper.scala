...
class SendEmailToNewUserFriendsHelper @Inject() (
    val userRepo: UserRepo,
    val postOffice: LocalPostOffice,
    val emailOptOutCommander: EmailOptOutCommander,
    val emailRepo: UserEmailAddressRepo,
...
class SendEmailToNewUserContactsHelper @Inject() (
    val userRepo: UserRepo,
    val postOffice: LocalPostOffice,
    val emailOptOutCommander: EmailOptOutCommander,
    val emailRepo: UserEmailAddressRepo,
  ...
  case class Sender(newUser: User, toNotify: Set[Id[User]]) extends NewUserEmailSender {
    ...
    def htmlBody(user: User): LargeString =
      views.html.email.contactJoinedInlined(user.firstName, newUser.firstName, newUser.lastName,
        newUser.externalId.toString, imageUrl, unsubLink(user)).body

    def textBody(user: User): Option[LargeString] =
      Some(views.html.email.contactJoinedText(user.firstName, newUser.firstName, newUser.lastName,
        newUser.externalId.toString, imageUrl, unsubLink(user)).body)
  }
