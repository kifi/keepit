  ...
  def suggestedLibrariesInfo(libId: Id[Library], userIdOpt: Option[Id[User]]): Future[(Seq[FullLibraryInfo], Seq[RelatedLibraryKind])] = {
    ...
    suggestedLibsFut
      ...
      .flatMap { relatedLibs =>
        ...
        val fullInfosFut = libraryInfoCommander.createFullLibraryInfos(userIdOpt, true, 10, 0, ProcessedImageSize.Large.idealSize, libs, ProcessedImageSize.Large.idealSize, withKeepTime = true, sanitizeUrls = false).map { _.map { _._2 } }
