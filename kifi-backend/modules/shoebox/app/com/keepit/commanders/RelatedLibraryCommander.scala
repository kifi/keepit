  ...
  def suggestedLibrariesInfo(libId: Id[Library], userIdOpt: Option[Id[User]]): Future[(Seq[FullLibraryInfo], Seq[RelatedLibraryKind])] = {
    ...
    suggestedLibsFut
      ...
      .flatMap { relatedLibs =>
        ...
        val fullInfosFut = libraryInfoCommander.createFullLibraryInfos(userIdOpt, false, 10, 0, 8, ProcessedImageSize.Large.idealSize, libs, ProcessedImageSize.Large.idealSize, sanitizeUrls = false).map { _.map { _._2 } }
