          ...
          case _ => {
            ...
            val library = db.readWrite { implicit s =>
              val lib = libraryRepo.save(Library(ownerId = ownerId, name = libAddReq.name, description = libAddReq.description,
                visibility = validVisibility, slug = validSlug, kind = LibraryKind.USER_CREATED, isSearchableByAnyone = libAddReq.isSearchableByOthers))
  ...
  def internSystemGeneratedLibraries(userId: Id[User]): (Library, Library) = { // returns true if created, false if already existed
    db.readWrite { implicit session =>
      ...
      val sysLibs = allLibs.filter(_._2.ownerId == userId)
        ...
        .map {
          case (kind, libs) =>
            ...
            val activeLib = libs.head._2.copy(state = LibraryStates.ACTIVE, slug = LibrarySlug(slug), name = name, visibility = LibraryVisibility.SECRET, isSearchableByAnyone = searchableByOthers)
      ...
      val mainOpt = if (sysLibs.find(_._2.kind == LibraryKind.SYSTEM_MAIN).isEmpty) {
        val mainLib = libraryRepo.save(Library(name = "Main Library", ownerId = userId, visibility = LibraryVisibility.SECRET, slug = LibrarySlug("main"), kind = LibraryKind.SYSTEM_MAIN, isSearchableByAnyone = true))
      ...
      val secretOpt = if (sysLibs.find(_._2.kind == LibraryKind.SYSTEM_SECRET).isEmpty) {
        val secretLib = libraryRepo.save(Library(name = "Secret Library", ownerId = userId, visibility = LibraryVisibility.SECRET, slug = LibrarySlug("secret"), kind = LibraryKind.SYSTEM_SECRET, isSearchableByAnyone = false))
