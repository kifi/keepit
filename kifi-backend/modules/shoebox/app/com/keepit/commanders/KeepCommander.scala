...
trait KeepCommander {
  ...
  def getBasicKeeps(ids: Set[Id[Keep]]): Map[Id[Keep], PersonalKeep]
  ...
  def getBasicKeeps(ids: Set[Id[Keep]]): Map[Id[Keep], PersonalKeep] = {
    db.readOnlyReplica { implicit session =>
      keepRepo.getByIds(ids).map {
        case (id, keep) => id -> PersonalKeep(keep.externalId, mine = false, removable = false, keep.visibility, Library.publicId(keep.libraryId.get))
      }
    }
  }
  def getKeepsCountFuture(): Future[Int] = {
