...
class RecommendationsCommander @Inject() (
  ...
  def topPublicLibraryRecos(userId: Id[User]): Future[Seq[FullRecoInfo]] = {
    ...
    libCommander.createFullLibraryInfos(Some(userId), false, maxMembersShown = 10, maxKeepsShown = 0, ProcessedImageSize.Large.idealSize, curatedLibraries).map { fullLibraryInfos =>
