    ...
    return {
      ...
      link: function (scope) {
        ...
        scope.hasKeepers = function (card) {
          return card.item && card.item.keepers && (card.item.keepers.length > 0);
        };

        scope.hasOthers = function (card) {
          return card.item && card.item.others > 0;
        };

        scope.getFriendText = function (card) {
          var num = (card.item && card.item.keepers) ? card.item.keepers.length : 0;
          var text = (num === 1) ? '1 friend' : num + ' friends';
          return (!card.isMyBookmark) ? text : 'and ' + text;
        };

        scope.getOthersText = function (card) {
          var num = (card.item && card.item.others) ? card.item.others : 0;
          var text = (num === 1) ? '1 other' : num + ' others';
          return (card.isMyBookmark || card.item.keepers.length > 0) ? 'and ' + text : text;
        };
      }
    };
  }
