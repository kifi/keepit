  ...
  "Scraper" should {
    "get a article from an existing website" in {
      withDb() { implicit injector =>
        ...
        val uri = normalizedURIFactory.apply(title = "title", url = url, state = NormalizedURIStates.SCRAPE_WANTED).copy(id = Some(Id(33)))
    ...
    "throw an error from a non-existing website" in {
      withDb() { implicit injector =>
        ...
        val uri = normalizedURIFactory.apply(title = "title", url = url, state = NormalizedURIStates.SCRAPE_WANTED).copy(id = Some(Id(44)))
        val result = scraper.fetchArticle(uri, info = ScrapeInfo(uriId = uri.id.get))
    ...
    "fetch allActive" in {
      withDb() { implicit injector =>
        inject[Database].readWrite { implicit s =>
          ...
          val uri1 = uriRepo.save(normalizedURIFactory.apply(title = "existing", url = "http://www.keepit.com/existing")
              .withState(NormalizedURIStates.SCRAPED))
          val uri2 = uriRepo.save(normalizedURIFactory.apply(title = "missing", url = "http://www.keepit.com/missing")
              .withState(NormalizedURIStates.SCRAPED))
    ...
    "fetch SCRAPE_WANTED uris and scrape them" in {
      withDb() { implicit injector =>
        ...
        val (uri1, uri2, info1, info2) = inject[Database].readWrite { implicit s =>
          val uri1 = uriRepo.save(normalizedURIFactory.apply(title = "existing", url = "http://www.keepit.com/existing", state = NormalizedURIStates.SCRAPE_WANTED))
          val uri2 = uriRepo.save(normalizedURIFactory.apply(title = "missing", url = "http://www.keepit.com/missing", state = NormalizedURIStates.SCRAPE_WANTED))
          val info1 = scrapeRepo.getByUri(uri1.id.get).get
          val info2 = scrapeRepo.getByUri(uri2.id.get).get
    ...
    "not scrape a not-modified page" in {
      ...
      withDb() { implicit injector =>
        ...
        var (uri1, info1) = inject[Database].readWrite { implicit s =>
          val uri1 = uriRepo.save(normalizedURIFactory.apply(title = "notModified", url = "http://www.keepit.com/notModified", state = NormalizedURIStates.SCRAPE_WANTED))
          val info1 = scrapeRepo.getByUri(uri1.id.get).get
          (uri1, info1)
        }
        val store = new FakeArticleStore()
    ...
    "update scrape schedule upon state change" in {
      withDb() { implicit injector =>
        ...
        var info = inject[Database].readWrite { implicit s =>
          val uri = uriRepo.save(normalizedURIFactory.apply(title = "existing", url = "http://www.keepit.com/existing", state = NormalizedURIStates.SCRAPE_WANTED))
