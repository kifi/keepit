    ...
    "clean up uris by changed uris" in {
      running(new ShoeboxApplication(modules: _*)) {
        ...
        val bms = db.readWrite { implicit session =>
          val bm0 = bmRepo.save(Keep(title = Some("google"), userId = user.id.get, url = urls(0).url, urlId = urls(0).id.get, uriId = uris(0).id.get, source = hover, libraryId = Some(lib1.id.get), libraryExternalId = Some(lib1.externalId)))
          val bm1 = bmRepo.save(Keep(title = Some("bing"), userId = user.id.get, url = urls(1).url, urlId = urls(1).id.get, uriId = uris(1).id.get, source = hover, libraryId = Some(lib1.id.get), libraryExternalId = Some(lib1.externalId)))
          val bm2 = bmRepo.save(Keep(title = Some("yahoo"), userId = user.id.get, url = urls(2).url, urlId = urls(2).id.get, uriId = uris(2).id.get, source = hover, libraryId = Some(lib1.id.get), libraryExternalId = Some(lib1.externalId)))
          val bm3 = bmRepo.save(Keep(title = Some("ask"), userId = user.id.get, url = urls(3).url, urlId = urls(3).id.get, uriId = uris(3).id.get, source = hover, libraryId = Some(lib1.id.get), libraryExternalId = Some(lib1.externalId)))
    ...
    "clean up uris by bookmarks" in {
      running(new ShoeboxApplication(modules: _*)) {
        ...
        bms ++= db.readWrite { implicit session =>
          ...
          Seq(bmRepo.save(Keep(title = Some("Yahoo"), userId = user.id.get, url = urls(2).url, urlId = urls(2).id.get, uriId = uris(2).id.get, source = hover, libraryId = Some(lib1.id.get), libraryExternalId = Some(lib1.externalId))))
        }
        cleaner.clean(readOnly = false)
        db.readOnlyMaster { implicit s =>
          uriRepo.get(uris(0).id.get).state === NormalizedURIStates.SCRAPED
        ...
        bms ++= db.readWrite { implicit session =>
          Seq(bmRepo.save(Keep(title = Some("AltaVista"), userId = user.id.get, url = urls(3).url, urlId = urls(3).id.get, uriId = uris(3).id.get, source = hover, libraryId = Some(lib1.id.get), libraryExternalId = Some(lib1.externalId))))
        }
        db.readOnlyMaster { implicit s =>
          uriRepo.get(uris(3).id.get).state === NormalizedURIStates.INACTIVE
        }
        ...
        val obms = db.readWrite { implicit s =>
          ...
          Seq(
            bmRepo.save(Keep(title = Some("google"), userId = other.id.get, url = urls(0).url, urlId = urls(0).id.get, uriId = uris(0).id.get, source = hover, libraryId = Some(lib1.id.get), libraryExternalId = Some(lib1.externalId))),
            bmRepo.save(Keep(title = Some("bing"), userId = other.id.get, url = urls(1).url, urlId = urls(1).id.get, uriId = uris(1).id.get, source = hover, libraryId = Some(lib1.id.get), libraryExternalId = Some(lib1.externalId)))
          )
        }
        cleaner.clean(readOnly = false)
        db.readOnlyMaster { implicit s =>
