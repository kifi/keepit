    ...
    "clean up uris by changed uris" in {
      running(new ShoeboxApplication(TestActorSystemModule(), ProdScrapeSchedulerModule(), TestScraperServiceClientModule(), FakeShoeboxServiceModule(), FakeAirbrakeModule())) {
        ...
        val bms = db.readWrite { implicit session =>
          val bm0 = bmRepo.save(Keep(title = Some("google"), userId = user.id.get, url = urls(0).url, urlId = urls(0).id,  uriId = uris(0).id.get, source = hover))
          val bm1 = bmRepo.save(Keep(title = Some("bing"), userId = user.id.get, url = urls(1).url, urlId = urls(1).id, uriId = uris(1).id.get, source = hover))
          val bm2 = bmRepo.save(Keep(title = Some("yahoo"), userId = user.id.get, url = urls(2).url, urlId = urls(2).id,  uriId = uris(2).id.get, source = hover))
          val bm3 = bmRepo.save(Keep(title = Some("ask"), userId = user.id.get, url = urls(3).url, urlId = urls(3).id, uriId = uris(3).id.get, source = hover))
    ...
    "clean up uris by bookmarks" in {
      running(new ShoeboxApplication(TestActorSystemModule(), ProdScrapeSchedulerModule(), TestScraperServiceClientModule(), FakeShoeboxServiceModule(), FakeAirbrakeModule())) {
        ...
        bms ++= db.readWrite { implicit session =>
          ...
          Seq(bmRepo.save(Keep(title = Some("Yahoo"), userId = user.id.get, url = urls(2).url, urlId = urls(2).id,  uriId = uris(2).id.get, source = hover)))
        }
        cleaner.clean(readOnly = false)
        db.readOnly{ implicit s =>
          uriRepo.get(uris(0).id.get).state === NormalizedURIStates.SCRAPED
        ...
        bms ++= db.readWrite { implicit session =>
          Seq(bmRepo.save(Keep(title = Some("AltaVista"), userId = user.id.get, url = urls(3).url, urlId = urls(3).id,  uriId = uris(3).id.get, source = hover)))
        }
        db.readOnly{ implicit s =>
          uriRepo.get(uris(3).id.get).state === NormalizedURIStates.INACTIVE
        }
        ...
        val obms = db.readWrite{ implicit s =>
          ...
          uriRepo.save(uris(1).withState(NormalizedURIStates.SCRAPE_FAILED))
          Seq(
            bmRepo.save(Keep(title = Some("google"), userId = other.id.get, url = urls(0).url, urlId = urls(0).id,  uriId = uris(0).id.get, source = hover)),
            bmRepo.save(Keep(title = Some("bing"), userId = other.id.get, url = urls(1).url, urlId = urls(1).id, uriId = uris(1).id.get, source = hover))
          )
        }
        cleaner.clean(readOnly = false)
        db.readOnly{ implicit s =>
