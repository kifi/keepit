...
class ScrapeInfoIntegrityCheckerTest extends Specification with ShoeboxTestInjector {
  ...
  "ScrapeInfoIntegrityChecker" should {

    "maintain ScrapeInfo" in {
      withDb(modules: _*) { implicit injector =>
        val db = inject[Database]
        val urlRepo = inject[URLRepo]
        val uriRepo = inject[NormalizedURIRepo]
        val keepRepo = inject[KeepRepo]
        val checker = inject[ScrapeInfoIntegrityChecker]
        checker.forceSeqNum(SequenceNumber[NormalizedURI](0L))
        ...
        Await.result(checker.checkIntegrity(), Duration("10 seconds"))
        ...
        db.readOnlyMaster { implicit s =>
          ...
          scrapeInfoRepo.getByUriId(uris(4).id.get).get.state === ScrapeInfoStates.ACTIVE
          scrapeInfoRepo.getByUriId(uris(5).id.get).get.state === ScrapeInfoStates.ACTIVE
          scrapeInfoRepo.getByUriId(uris(6).id.get) === None
          scrapeInfoRepo.getByUriId(uris(7).id.get) === None
        }
        ...
        db.readWrite { implicit s =>
          bms.foreach { bm =>
            keepRepo.save(bm.withState(KeepStates.INACTIVE))
          }
          uriRepo.save(uris(0).withState(NormalizedURIStates.ACTIVE))
          uriRepo.save(uris(1).withState(NormalizedURIStates.ACTIVE))
          uriRepo.save(uris(4).withState(NormalizedURIStates.ACTIVE))
          uriRepo.save(uris(5).withState(NormalizedURIStates.ACTIVE))
        ...
        db.readOnlyMaster { implicit s =>
          ...
          scrapeInfoRepo.getByUriId(uris(2).id.get).get.state === ScrapeInfoStates.ACTIVE
          scrapeInfoRepo.getByUriId(uris(3).id.get).get.state === ScrapeInfoStates.ACTIVE
          scrapeInfoRepo.getByUriId(uris(4).id.get).get.state === ScrapeInfoStates.INACTIVE
          scrapeInfoRepo.getByUriId(uris(5).id.get).get.state === ScrapeInfoStates.INACTIVE
          scrapeInfoRepo.getByUriId(uris(6).id.get) === None
          scrapeInfoRepo.getByUriId(uris(7).id.get) === None
        }

        db.readWrite { implicit s =>
          uriRepo.save(uris(3).withState(NormalizedURIStates.ACTIVE))
          uriRepo.assignSequenceNumbers(1000)
        }
