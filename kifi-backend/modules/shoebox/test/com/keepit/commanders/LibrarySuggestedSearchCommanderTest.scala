...
class LibrarySuggestedSearchCommanderTest extends Specification with ShoeboxTestInjector {
  ...
  def setup(implicit injector: Injector) {
    ...
    val t1 = LibrarySuggestedSearch(term = "a", weight = 1f, libraryId = Id[Library](1))
  ...
  "LibrarySuggestedSearchCommander" should {
    ...
    "retrieve suggested terms in library" in {
      withDb() { implicit injector =>
        ...
        val id = Id[Library](1)
        var ts = commander.getSuggestedTermsForLibrary(id, limit = 1)
        ts.terms === Map("b" -> 2f)

        ts = commander.getSuggestedTermsForLibrary(id, limit = 2)
        ts.terms === Map("b" -> 2f, "a" -> 1f)

        ts = commander.getSuggestedTermsForLibrary(id, limit = 3)
        ts.terms === Map("b" -> 2f, "a" -> 1f)
      }
    }
    ...
    "save new terms" in {
      withDb() { implicit injector =>
        ...
        val id = Id[Library](1)
        commander.saveSuggestedSearchTermsForLibrary(id, SuggestedSearchTerms(newTerms))
        var ts = commander.getSuggestedTermsForLibrary(id, limit = 1)
        ts.terms === Map("e" -> 5f)
        ...
        ts = commander.getSuggestedTermsForLibrary(id, limit = 4)
        ts.terms === Map("e" -> 5f, "c" -> 3.5f, "a" -> 1.5f)
