  ...
  "KeepsCommander" should {
    ...
    "filter fake users and libs" in {
      withDb(modules: _*) { implicit injector =>
        ...
        val seq1 = Seq(LimitedAugmentationInfo(None, Seq.empty, 0, 0, Seq((real, now)), 0, 0, Seq((Id[Library](1), real, now)), 0, 0, Seq.empty, 0),
          LimitedAugmentationInfo(None, Seq.empty, 0, 0, Seq((real, now)), 0, 0, Seq((Id[Library](2), real, now)), 0, 0, Seq.empty, 0),
          LimitedAugmentationInfo(None, Seq.empty, 0, 0, Seq(), 0, 0, Seq(), 0, 0, Seq.empty, 0))
        inject[KeepDecorator].filterLibraries(seq1) === seq1
        val seq2 = Seq(LimitedAugmentationInfo(None, Seq.empty, 0, 0, Seq((real, now)), 0, 0, Seq((Id[Library](1), real, now)), 0, 0, Seq.empty, 0),
          LimitedAugmentationInfo(None, Seq.empty, 0, 0, Seq((real, now), (fake, now)), 0, 0, Seq((Id[Library](2), real, now), (Id[Library](3), fake, now)), 0, 0, Seq.empty, 0),
          LimitedAugmentationInfo(None, Seq.empty, 0, 0, Seq(), 0, 0, Seq((Id[Library](3), fake, now)), 0, 0, Seq.empty, 0))
