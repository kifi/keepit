    ...
    "serve up full library infos" in {
      "not fail for org general libraries" in {
        withDb(modules: _*) { implicit injector =>
          ...
          val fullInfosForOwner = inject[LibraryInfoCommander].createFullLibraryInfo(Some(owner.id.get), showPublishedLibraries = true, orgGeneralLibrary, ProcessedImageSize.Large.idealSize, None, withKeepTime = true, sanitizeUrls = false, useMultilibLogic = true)
          val fullInfosForMember = inject[LibraryInfoCommander].createFullLibraryInfo(Some(member.id.get), showPublishedLibraries = true, orgGeneralLibrary, ProcessedImageSize.Large.idealSize, None, withKeepTime = true, sanitizeUrls = false, useMultilibLogic = true)
