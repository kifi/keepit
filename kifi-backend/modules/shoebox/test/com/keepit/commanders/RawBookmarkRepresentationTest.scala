...
class RawBookmarkRepresentationTest extends Specification {
  "BookmarkInterner" should {
    ...
    "getBookmarkJsonObjects" in {
      val raw = new RawBookmarkFactory(null).getBookmarkJsonObjects(Json.parse("""{"bookmarks":[{"title":"Presto | Distributed SQL Query Engine for Big Data","url":"http://prestodb.io/","isPrivate":false}],"source":"HOVER_KEEP"}"""))
      raw.size === 1
