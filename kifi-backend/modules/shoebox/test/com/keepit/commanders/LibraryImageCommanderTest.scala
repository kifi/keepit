  ...
  "LibraryImageCommander" should {
    ...
    "upload & remove library image" in {
      withDb(modules: _*) { implicit injector =>
        ...
        val savedF = commander.setLibraryImageFromFile(file1, lib.id.get, None, ImageSource.UserUpload)
        ...
        db.readOnlyMaster { implicit s =>
          ...
          libraryImages.map(_.imagePosition).flatten.length === 0
        }
    ...
    "upload image then adjust position" in {
      withDb(modules: _*) { implicit injector =>
        ...
        val newSizing = LibraryImagePosition(50, 50, 30, 30)
        commander.positionLibraryImage(lib.id.get, newSizing)
