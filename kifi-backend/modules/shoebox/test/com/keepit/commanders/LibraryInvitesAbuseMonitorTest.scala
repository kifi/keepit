...
class LibraryInvitesAbuseMonitorTest extends Specification with ShoeboxTestInjector {
  ...
  def setup()(implicit injector: Injector) = {
    db.readWrite { implicit s =>
      val u1 = userRepo.save(User(firstName = "Aaron", lastName = "H", username = Username("test1"), normalizedUsername = "test1", pictureName = Some("0")))
      val u2 = userRepo.save(User(firstName = "Baron", lastName = "H", username = Username("test2"), normalizedUsername = "test2", pictureName = Some("0")))
