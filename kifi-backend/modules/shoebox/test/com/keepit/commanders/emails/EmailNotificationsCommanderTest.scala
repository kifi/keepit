    ...
    "send email" in {
      withDb(FakeMailModule()) { implicit injector =>
        ...
        val (link, william, abraham, george) = inject[Database].readWrite { implicit session =>
          val william = userRepo.save(User(firstName = "William", lastName = "Shakespeare", username = Username("test"), normalizedUsername = "test"))
          val george = userRepo.save(User(firstName = "George", lastName = "Washington", username = Username("test"), normalizedUsername = "test"))
          ...
          (deepLinkRepo.save(link),
            william,
            userRepo.save(User(firstName = "Abraham", lastName = "Lincoln", username = Username("test"), normalizedUsername = "test")),
            george)
        }
