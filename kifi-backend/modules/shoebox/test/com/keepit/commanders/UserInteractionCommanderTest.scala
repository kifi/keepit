    ...
    "calculate interaction score" in {
      withDb(modules: _*) { implicit injector =>
        ...
        val (user1, user2, user3, user4) = db.readWrite { implicit session =>
          val user1 = userRepo.save(User(firstName = "George", lastName = "Washington", username = Username("GDubs"), normalizedUsername = "a", pictureName = Some("0")))
          val user2 = userRepo.save(User(firstName = "Abe", lastName = "Lincoln", username = Username("VampireXSlayer"), normalizedUsername = "b", pictureName = Some("0")))
          val user3 = userRepo.save(User(firstName = "Ben", lastName = "Franklin", username = Username("Benji"), normalizedUsername = "c", pictureName = Some("0")))
