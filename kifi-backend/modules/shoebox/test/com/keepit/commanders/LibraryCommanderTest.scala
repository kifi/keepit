    ...
    "when modify library is called:" in {
      ...
      "let org members with the force-edit permission move libraries" in {
        withDb(modules: _*) { implicit injector =>
          ...
          val newSettings = db.readOnlyMaster { implicit session => orgConfigRepo.getByOrgId(org.id.get).settings.withFeatureSetTo(Feature.ForceEditLibraries -> FeatureSelection.ADMINS) }
