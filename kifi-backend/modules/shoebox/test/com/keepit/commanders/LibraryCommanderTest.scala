  ...
  "LibraryCommander" should {
    ...
    "can move a library to and from organization space" in {
      withDb(modules: _*) { implicit injector =>
        ...
        libraryCommander.canMoveToFromOrg(Id[User](0), newLibrary.id.get, organization.id) must equalTo(false)

        // User owns the library
        // Can move libraries to organizations you are part of.
        libraryCommander.canMoveToFromOrg(user.id.get, newLibrary.id.get, organization.id) must equalTo(true)
        // Cannot inject libraries to random organizations.
        libraryCommander.canMoveToFromOrg(user.id.get, newLibrary.id.get, otherOrg.id) must equalTo(false)
        ...
        libraryCommander.canMoveToFromOrg(user.id.get, newLibrary.id.get, None) must equalTo(true)
        ...
        libraryCommander.canMoveToFromOrg(user.id.get, newLibrary.id.get, None) must equalTo(false)
        // Prevent Company Espionage and library stealing!!
        libraryCommander.canMoveToFromOrg(user.id.get, newLibrary.id.get, organization.id) must equalTo(false)
