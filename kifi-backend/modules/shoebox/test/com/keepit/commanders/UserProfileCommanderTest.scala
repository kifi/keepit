  ...
  "getConnectionsSortedByRelationship" in {
    withDb(modules: _*) { implicit injector =>
      ...
      val relationship = SociallyRelatedEntitiesForUser(
        RelatedEntities[User, User](owner.id.get, Seq(user4.id.get -> 1, user5.id.get -> 4, user2.id.get -> 2, user3.id.get -> 3), totalSteps = 10),
        RelatedEntities[User, SocialUserInfo](owner.id.get, Seq.empty, totalSteps = 0),
        RelatedEntities[User, SocialUserInfo](owner.id.get, Seq.empty, totalSteps = 0),
        RelatedEntities[User, EmailAccountInfo](owner.id.get, Seq.empty, totalSteps = 0),
        RelatedEntities[User, Organization](owner.id.get, Seq.empty, totalSteps = 0),
        totalSteps = 100000
      )
