...
class KeepExportTest extends Specification with ShoeboxTestInjector {
    ...
    "export keeps" in {
      withDb() { implicit injector =>
        ...
        db.readOnly { implicit s =>
          ...
          keepExports(0) === KeepExport(title = Some("k3"), created_at = t1.plusMinutes(6), url = site3)
          keepExports(1) === KeepExport(title = Some("k2"), created_at = t1.plusMinutes(9), url = site2)
          keepExports(2) === KeepExport(title = Some("k1"), created_at = t1.plusMinutes(3), url = site1, tags = Some("t1"))
        }
      }
    }
