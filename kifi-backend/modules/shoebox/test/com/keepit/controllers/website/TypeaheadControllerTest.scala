  ...
  "TypeaheadController" should {
    ...
    "query connections" in {
      running(new ShoeboxApplication(controllerTestModules: _*)) {
        ...
        {
          val path = com.keepit.controllers.website.routes.TypeaheadController.searchWithInviteStatus(Some("杨"), Some(10), false, false).toString
          path === "/site/user/connections/all/search?query=%E6%9D%A8&limit=10&pictureUrl=false&dedupEmail=false"
          val res = route(FakeRequest("GET", path)).get
        ...
        }
        {
          ...
          path === "/site/user/connections/all/search?query=leo&&limit=210&pictureUrl=false&dedupEmail=false"
          val res = route(FakeRequest("GET", path)).get
          getNames(res) must_== Seq("Léo Grimaldi")
        }
        {
          val path = com.keepit.controllers.website.routes.TypeaheadController.searchWithInviteStatus(None, Some(2), false, false).toString
          path === "/site/user/connections/all/search?%2Farst&limit=2"
          val res = route(FakeRequest("GET", path)).get
          getNames(res) must_== Seq("杨莹")
        }
      }
