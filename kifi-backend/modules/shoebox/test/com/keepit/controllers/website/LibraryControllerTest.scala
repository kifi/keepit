      ...
      withDb(modules: _*) { implicit injector =>
        ...
        (resultJson \ "library" \ "org").as[Option[BasicOrganizationView]] must equalTo(Some(
          BasicOrganizationView(
            ...
            OrganizationViewerInfo(invite = None, permissionCommander.settinglessOrganizationPermissions(Some(OrganizationRole.ADMIN)), Some(OrganizationMembershipInfo(OrganizationRole.ADMIN)))
          )
        ))
