  ...
  "processAppStoreParams" in {
    withDb(controllerTestModules: _*) { implicit injector =>
      val (user1, user2) = db.readWrite { implicit s =>
        val user1 = userRepo.save(User(firstName = "Andrew", lastName = "C", username = Username("test1"), normalizedUsername = "test1", pictureName = Some("0")))
        val user2 = userRepo.save(User(firstName = "Eishay", lastName = "S", username = Username("test"), normalizedUsername = "test", pictureName = Some("0")))
