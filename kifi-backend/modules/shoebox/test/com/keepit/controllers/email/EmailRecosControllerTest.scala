      ...
      def testViewRecoAction(isAuthenticated: Boolean) = {
        withDb(controllerTestModules: _*) { implicit injector =>
          ...
          val (userOpt, uri1) = db.readWrite { implicit s =>
            val userOpt = if (isAuthenticated) Some(UserFactory.user().withName("Jo", "Bennett").withUsername("test").saved) else None
            uriRepo.count === 0
          ...
          if (isAuthenticated) {
            ...
            inject[LibraryInfoCommander].internSystemGeneratedLibraries(userOpt.get.id.get)
          }
