    ...
    "handle valid requests from various users and devices" in {
      withDb(FakeUserActionsModule()) { implicit injector =>
        ...
        val (heinlein, niven, uri, deepLink) = db.readWrite { implicit s =>
          ...
          val heinlein = userRepo.save(User(firstName = "Robert", lastName = "Heinlein", username = Username("test"), normalizedUsername = "test", pictureName = Some("0")))
          val niven = userRepo.save(User(firstName = "Larry", lastName = "Niven", username = Username("test2"), normalizedUsername = "test2", pictureName = Some("0")))
