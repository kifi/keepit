    ...
    "save and load" in {
      withDb() { implicit injector =>
        ...
        val (u1, u2, d1, d2, u1d1, u1d2, u2d2) = inject[Database].readWrite { implicit session =>
          val u1 = inject[UserRepo].save(User(firstName = "J", lastName = "Jacobs", username = Username("test"), normalizedUsername = "test", pictureName = Some("0")))
          val u2 = inject[UserRepo].save(User(firstName = "A", lastName = "Jacobs", username = Username("test2"), normalizedUsername = "test2", pictureName = Some("0")))
