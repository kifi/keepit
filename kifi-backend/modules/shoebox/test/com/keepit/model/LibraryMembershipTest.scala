    ...
    "basically work" in { // test read/write/save
      withDb() { implicit injector =>
        ...
        db.readOnlyMaster { implicit session =>
          ...
          libraryMembershipRepo.countNonTrivialLibrariesWithUserIdAndAccess(user1.id.get, LibraryAccess.READ_ONLY) === 0
          libraryMembershipRepo.countNonTrivialLibrariesWithUserIdAndAccess(user2.id.get, LibraryAccess.OWNER) === 1
          libraryMembershipRepo.countNonTrivialLibrariesWithUserIdAndAccess(user2.id.get, LibraryAccess.READ_ONLY) === 1
        }
      }
    }
