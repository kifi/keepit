...
class LibraryMembershipTest extends Specification with ShoeboxTestInjector {
  ...
  def setup()(implicit injector: Injector) = {
    ...
    db.readWrite { implicit s =>
      ...
      val library1 = libraryRepo.save(Library(name = "Lib1", ownerId = user1.id.get, createdAt = t1.plusMinutes(2), keepCount = 1, lastKept = Some(t1.plusMinutes(2)),
        visibility = LibraryVisibility.PUBLISHED, slug = LibrarySlug("A"), memberCount = 1))
      val library2 = libraryRepo.save(Library(name = "Lib2", ownerId = user2.id.get, createdAt = t1.plusMinutes(5), keepCount = 1, lastKept = Some(t1.plusMinutes(2)),
