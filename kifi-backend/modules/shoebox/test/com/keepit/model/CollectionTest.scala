  ...
  def setup()(implicit injector: Injector) = {
    ...
    db.readWrite { implicit s =>
      ...
      val bookmark1 = keepRepo.save(Keep(title = Some("G1"), userId = user1.id.get, url = url1.url,
        urlId = url1.id.get, uriId = uri1.id.get, source = hover, createdAt = t1.plusMinutes(3), libraryId = Some(lib1.id.get), libraryExternalId = Some(lib1.externalId)))
      val bookmark2 = keepRepo.save(Keep(title = Some("A1"), userId = user1.id.get, url = url2.url,
        urlId = url2.id.get, uriId = uri2.id.get, source = hover, createdAt = t1.plusHours(50), libraryId = Some(lib1.id.get), libraryExternalId = Some(lib1.externalId)))
