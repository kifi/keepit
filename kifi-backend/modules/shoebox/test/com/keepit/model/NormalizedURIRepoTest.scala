...
class NormalizedURIRepoTest extends Specification with ShoeboxTestInjector {
  ...
  def setup()(implicit injector: Injector) = {
    db.readWrite { implicit s =>
      ...
      keepRepo.save(KeepFactory(url1.url, uri = uri1, userId = user1.id.get, title = Some("my title is short"), url = url1, source = KeepSource("NA"), libraryId = Some(lib1.id.get), libraryExternalId = Some(lib1.externalId)))
      keepRepo.save(KeepFactory(url2.url, uri = uri2, userId = user1.id.get, title = Some("my title is long"), url = url2, source = KeepSource("NA"), libraryId = Some(lib1.id.get), libraryExternalId = Some(lib1.externalId)))
      keepRepo.save(KeepFactory(url2.url, uri = uri2, userId = user2.id.get, title = Some("my title is long"), url = url2, source = KeepSource("NA"), libraryId = Some(lib1.id.get), libraryExternalId = Some(lib1.externalId)))
    }
  }
