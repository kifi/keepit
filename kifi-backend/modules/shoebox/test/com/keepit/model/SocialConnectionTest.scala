  ...
  "SocialConnection" should {
    ...
    "give Kifi user's connections (min set)" in {
      withDb(socialConnectionTestModules: _*) { implicit injector =>
        ...
        val (eishaySocialUserInfo, andrewSocialUserInfo) = inject[Database].readWrite { implicit s =>
          (
            socialRepo.save(socialRepo.get(SocialId("646386018"), SocialNetworks.FACEBOOK).withUser(userRepo.save(User(firstName = "Eishay", lastName = "Smith", username = Username("test"), normalizedUsername = "test", pictureName = Some("0"))))),
            socialRepo.save(socialRepo.get(SocialId("71105121"), SocialNetworks.FACEBOOK).withUser(userRepo.save(User(firstName = "Andrew", lastName = "Conner", username = Username("test2"), normalizedUsername = "test2", pictureName = Some("0")))))
          )
        }

        val eishayJson = Json.parse(io.Source.fromFile(new File("test/com/keepit/common/social/data/%s".format("facebook_graph_eishay_min.json"))).mkString)
        val andrewJson = Json.parse(io.Source.fromFile(new File("test/com/keepit/common/social/data/%s".format("facebook_graph_andrew_min.json"))).mkString)
        ...
        val users = inject[Database].readWrite { implicit s =>
          ...
          val users = userRepo.save(User(firstName = "Igor", lastName = "Perisic", username = Username("test"), normalizedUsername = "test", pictureName = Some("0"))) ::
            userRepo.save(User(firstName = "Kelvin", lastName = "Jiang", username = Username("test2"), normalizedUsername = "test2", pictureName = Some("0"))) ::
            userRepo.save(User(firstName = "John", lastName = "Cochran", username = Username("test3"), normalizedUsername = "test3", pictureName = Some("0"))) :: Nil
    ...
    "give Kifi user's connections (min set) w/o non active connections" in {
      withDb(socialConnectionTestModules: _*) { implicit injector =>
        ...
        val (socialUser, andrewUser: User) = inject[Database].readWrite { implicit s =>
          val u: User = inject[UserRepo].save(User(firstName = "Andrew", lastName = "Conner", username = Username("test"), normalizedUsername = "test", pictureName = Some("0")))
          val su = inject[SocialUserInfoRepo].save(SocialUserInfo(
            fullName = "Andrew Conner",
            socialId = SocialId("71105121"),
            networkType = SocialNetworks.FACEBOOK
        ...
        val (eishaySocialUserInfo, eishayUser) = inject[Database].readWrite { implicit s =>
          var user = userRepo.save(User(firstName = "Eishay", lastName = "Smith", username = Username("test"), normalizedUsername = "test", pictureName = Some("0")))
        ...
        val (andrewSocialUserInfo, andrewUser2) = inject[Database].readWrite { implicit s =>
          var user = userRepo.save(User(firstName = "Andrew", lastName = "Conner", username = Username("test"), normalizedUsername = "test", pictureName = Some("0")))
        ...
        val users = inject[Database].readWrite { implicit s =>
          val users = userRepo.save(User(firstName = "Igor", lastName = "Perisic", username = Username("test"), normalizedUsername = "test", pictureName = Some("0"))) ::
            userRepo.save(User(firstName = "Kelvin", lastName = "Jiang", username = Username("test"), normalizedUsername = "test", pictureName = Some("0"))) ::
            userRepo.save(User(firstName = "John", lastName = "Cochran", username = Username("test"), normalizedUsername = "test", pictureName = Some("0"))) :: Nil
    ...
    "give Kifi user's connections (min set) with pagination" in {
      withDb(socialConnectionTestModules: _*) { implicit injector =>
        ...
        val eishay1Json = Json.parse(io.Source.fromFile(new File("test/com/keepit/common/social/data/%s".format("facebook_graph_eishay_min_page1.json"))).mkString)
        val eishay2Json = Json.parse(io.Source.fromFile(new File("test/com/keepit/common/social/data/%s".format("facebook_graph_eishay_min_page2.json"))).mkString)

        // Create FortyTwo accounts on certain users
        val users = inject[Database].readWrite { implicit s =>
          val users = userRepo.save(User(firstName = "Igor", lastName = "Perisic", username = Username("test"), normalizedUsername = "test", pictureName = Some("0"))) ::
            userRepo.save(User(firstName = "Kelvin", lastName = "Jiang", username = Username("test2"), normalizedUsername = "test2", pictureName = Some("0"))) ::
            userRepo.save(User(firstName = "John", lastName = "Cochran", username = Username("test3"), normalizedUsername = "test3", pictureName = Some("0"))) ::
            userRepo.save(User(firstName = "Andrew", lastName = "Conner", username = Username("test4"), normalizedUsername = "test4", pictureName = Some("0"))) :: Nil
