    ...
    "save organizations and get them by id" in {
      withDb() { implicit injector =>
        ...
        val org = db.readWrite { implicit s =>
          orgRepo.save(Organization(name = "Bob", ownerId = Id[User](1), primaryHandle = Some(PrimaryOrganizationHandle(OrganizationHandle("handle"), OrganizationHandle("handle"))), description = None, site = None))
        }
