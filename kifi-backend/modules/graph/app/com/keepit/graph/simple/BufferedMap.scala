...
class BufferedMap[A, B](val current: MutableMap[A, B], val updated: MutableMap[A, B] = MutableMap[A, B](), val removed: MutableSet[A] = MutableSet[A]()) extends MutableMap[A, B] {
