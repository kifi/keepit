...
trait URIWanderingTestHelper {
  ...
  def genUserGraphUpdate(i: Int, firstName: String, lastName: String) = UserGraphUpdate(User(id = Some(Id[User](i)), firstName = firstName, lastName = lastName, seq = SequenceNumber(i), username = Username("test"), normalizedUsername = "test", pictureName = Some("0")))
