    ...
    "get list of user and score pairs" in {
      ...
      "get socially related entities for users" in {
        withInjector(modules: _*) { implicit injector =>
          ...
          sociallyRelatedEntities.facebookAccounts.normalizingConstant === 0.0
          sociallyRelatedEntities.linkedInAccounts.id === u42
          sociallyRelatedEntities.linkedInAccounts.normalizingConstant === 0.0
          sociallyRelatedEntities.emailAccounts.id === u42
          sociallyRelatedEntities.emailAccounts.normalizingConstant === sociallyRelatedEntities.emailAccounts.related.map(_._2).sum
          sociallyRelatedEntities.users.id === u42
          sociallyRelatedEntities.users.related.size === 5
          sociallyRelatedEntities.users.normalizingConstant === sociallyRelatedEntities.users.related.map(_._2).sum
        }
      }
      ...
      "get socially related entities for orgs" in {
        withInjector(modules: _*) { implicit injector =>
          ...
          sociallyRelatedEntities.emailAccounts.related.map(_._2).sum === sociallyRelatedEntities.emailAccounts.normalizingConstant
