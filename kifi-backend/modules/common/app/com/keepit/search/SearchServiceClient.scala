...
class SearchServiceClientImpl(
  ...
  def augment(userId: Option[Id[User]], showPublishedLibraries: Boolean, maxKeepsShown: Int, maxKeepersShown: Int, maxLibrariesShown: Int, maxTagsShown: Int, items: Seq[AugmentableItem]): Future[Seq[LimitedAugmentationInfo]] = {
    if (items.isEmpty) Future.successful(Seq.empty[LimitedAugmentationInfo])
      ...
      val payload = Json.obj("userId" -> userId, "showPublishedLibraries" -> showPublishedLibraries, "maxKeepsShown" -> maxKeepsShown, "maxKeepersShown" -> maxKeepersShown, "maxLibrariesShown" -> maxLibrariesShown, "maxTagsShown" -> maxTagsShown, "items" -> items)
