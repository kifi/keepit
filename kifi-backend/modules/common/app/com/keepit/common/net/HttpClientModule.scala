...
case class ProdHttpClientModule() extends HttpClientModule {
  ...
  @Provides
  def httpClientProvider(healthcheckPlugin: HealthcheckPlugin, airbrake: Provider[AirbrakeNotifier], services: FortyTwoServices): HttpClient =
    new HttpClientImpl(healthcheckPlugin = healthcheckPlugin, airbrake = airbrake, services = services)
}
