...
abstract class LocalDiscoveryModule(serviceType: ServiceType) extends DiscoveryModule {
  ...
  def serviceCluster(amazonInstanceInfo: AmazonInstanceInfo, airbrake: Provider[AirbrakeNotifier]): ServiceCluster =
    new ServiceCluster(serviceType, airbrake) tap { cluster =>
      cluster.register(new ServiceInstance(Node(cluster.serviceNodeMaster.path + "_0"), true).setRemoteService(RemoteService(amazonInstanceInfo, ServiceStatus.UP, serviceType)))
    }
  ...
  def serviceDiscovery(services: FortyTwoServices, amazonInstanceInfoProvider: Provider[AmazonInstanceInfo], cluster: ServiceCluster): ServiceDiscovery =
    new ServiceDiscovery {
      ...
      def thisInstance = Some(new ServiceInstance(Node(cluster.serviceNodeMaster.path + "_0"), true).setRemoteService(RemoteService(amazonInstanceInfoProvider.get, ServiceStatus.UP, cluster.serviceType)))
