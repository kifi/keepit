...
class GraphServiceClientImpl(
  ...
  def getSociallyRelatedEntitiesForUser(userId: Id[User]): Future[Option[SociallyRelatedEntitiesForUser]] = {
    ...
    cacheProvider.relatedEntitiesForUserCache.
      getOrElseFutureOpt(SociallyRelatedEntitiesForUserCacheKey(userId), needRefresh) {
        call(Graph.internal.getSociallyRelatedEntitiesForUser(userId), callTimeouts = longTimeout).map { r =>
          r.json.asOpt[SociallyRelatedEntitiesForUser]
        }
      }
  }
  ...
  def getSociallyRelatedEntitiesForOrg(orgId: Id[Organization]): Future[Option[SociallyRelatedEntitiesForOrg]] = {
    ...
    cacheProvider.relatedEntitiesForOrgCache.
      getOrElseFutureOpt(SociallyRelatedEntitiesForOrgCacheKey(orgId), needRefresh) {
        call(Graph.internal.getSociallyRelatedEntitiesForOrg(orgId), callTimeouts = longTimeout).map { r =>
          r.json.asOpt[SociallyRelatedEntitiesForOrg]
        }
      }
  }
