...
object PornDomains extends Logging {
  ...
  def isPornDomain(url: String): Boolean = {
    ...
    val res = domainRegexes.exists(reg => reg.findFirstMatchIn(urlLower).isDefined)
    log.info(s"porn domain tests took ${System.currentTimeMillis - t1} millis")
    res
  }
}
