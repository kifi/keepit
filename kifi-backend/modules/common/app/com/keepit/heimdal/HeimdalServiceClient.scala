...
trait HeimdalServiceClient extends ServiceClient {
  ...
  def postDelightedAnswer(userCreationInfo: DelightedUserCreationInfo, answer: BasicDelightedAnswer): Future[Option[BasicDelightedAnswer]]
...
class HeimdalServiceClientImpl @Inject() (
  ...
  def postDelightedAnswer(userCreationInfo: DelightedUserCreationInfo, answer: BasicDelightedAnswer): Future[Option[BasicDelightedAnswer]] = {
    call(Heimdal.internal.postDelightedAnswer(), Json.obj(
      "user" -> Json.toJson(userCreationInfo),
      "answer" -> Json.toJson(answer)
    )).map { response =>
      val json = Json.parse(response.body)
      json.asOpt[BasicDelightedAnswer] orElse {
        (json \ "error").asOpt[String].map { msg =>
          log.warn(s"Error posting delighted answer $answer for user ${userCreationInfo.userId}: $msg")
        }
        None
      }
    }
  ...
  def cancelDelightedSurvey(userCreationInfo: DelightedUserCreationInfo): Future[Boolean] = {
    call(Heimdal.internal.cancelDelightedSurvey(), Json.obj(
      ...
      Json.parse(response.body) match {
        ...
        case json =>
          (json \ "error").asOpt[String].map { msg =>
            log.warn(s"Error cancelling delighted survey for user ${userCreationInfo.userId}: $msg")
          } getOrElse {
            log.warn(s"Error cancelling delighted survey for user ${userCreationInfo.userId}")
          }
          false
      }
    }
