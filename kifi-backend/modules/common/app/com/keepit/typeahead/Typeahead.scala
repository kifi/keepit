...
trait Typeahead[T, E, I, P <: PersonalTypeahead[T, E, I]] extends Logging {
  ...
  def prefetch(ownerId: Id[T]): Future[Unit] = {
    getOrElseCreate(ownerId).map(_ => ())(ExecutionContect)
  }
...
@json
case class UserPrefixSearchRequest(userId: Id[User], query: String, maxHits: Int, acceptLangs: Seq[String], userExperiments: Set[UserExperimentType])
@json
case class UserPrefixSearchHit(userId: Id[User], score: Float)
