...
case class Organization(
  ...
  def nonmemberPermissions = basePermissions.forNonmember
  def rolePermissions(role: OrganizationRole) = basePermissions.forRole(role)

  def newMembership(userId: Id[User], role: OrganizationRole): OrganizationMembership =
    OrganizationMembership(organizationId = id.get, userId = userId, role = role, permissions = rolePermissions(role))

  def modifiedMembership(membership: OrganizationMembership, newRole: OrganizationRole): OrganizationMembership =
    membership.copy(role = newRole, permissions = rolePermissions(newRole))
}
