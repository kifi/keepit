...
object OrganizationFactory {
  ...
  case class PartialOrganization private[OrganizationFactory] (
      ...
      nonstandardSettings: Set[FeatureSettingPair[_ <: FeatureKind]] = Set.empty) {
    ...
    def withWeakMembers() = this.copy(nonstandardSettings = nonstandardSettings + FeatureSettingPair(Feature.InviteMembers, FeatureSetting.ADMINS))
    // admins can force-edit
    def withStrongAdmins() = this.copy(nonstandardSettings = nonstandardSettings + FeatureSettingPair(Feature.ForceEditLibraries, FeatureSetting.ADMINS))
    def secret() = this.copy(nonstandardSettings = nonstandardSettings + FeatureSettingPair(Feature.ViewOrganization, FeatureSetting.MEMBERS) + FeatureSettingPair(Feature.ViewMembers, FeatureSetting.MEMBERS))
  }
}
