...
class FakeHeimdalServiceClientImpl(val airbrakeNotifier: AirbrakeNotifier) extends HeimdalServiceClient with FakeServiceClient {
  ...
  def getGratData(userId: Id[User]): Future[GratificationData] = Future.successful(GratificationData(Id[User](1), LibraryCountData(0, Map.empty), KeepCountData(0, Map.empty), KeepCountData(0, Map.empty)))
