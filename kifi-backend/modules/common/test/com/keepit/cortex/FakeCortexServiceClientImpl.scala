...
class FakeCortexServiceClientImpl(val airbrakeNotifier: AirbrakeNotifier) extends CortexServiceClient {
  ...
  override def batchUserURIsInterests(userId: Id[User], uriIds: Seq[Id[NormalizedURI]])(implicit version: LDAVersionOpt): Future[Seq[LDAUserURIInterestScores]] = {
  ...
  override def sampleURIsForTopic(topic: Int)(implicit version: LDAVersionOpt = None): Future[(Seq[Id[NormalizedURI]], Seq[Float])] = ???
  ...
  override def getTopicNames(uris: Seq[Id[NormalizedURI]])(implicit version: LDAVersionOpt): Future[Seq[Option[String]]] = Future.successful(Seq.fill(uris.length)(None))
  override def explainFeed(userId: Id[User], uriIds: Seq[Id[NormalizedURI]])(implicit version: LDAVersionOpt): Future[Seq[Seq[Id[Keep]]]] = Future.successful(Seq.fill(uriIds.length)(Seq()))
