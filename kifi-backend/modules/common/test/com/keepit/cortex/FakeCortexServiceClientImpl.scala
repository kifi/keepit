...
class FakeCortexServiceClientImpl(val airbrakeNotifier: AirbrakeNotifier) extends CortexServiceClient {
  ...
  override def userUriInterest(userId: Id[User], uriId: Id[NormalizedURI]): Future[(Option[LDAUserURIInterestScore], Option[LDAUserURIInterestScore])] = Future.successful(None, None)
