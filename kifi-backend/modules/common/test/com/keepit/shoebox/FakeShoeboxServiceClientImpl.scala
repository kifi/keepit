  ...
  def saveBookmarksByEdges(edges: Seq[(NormalizedURI, User, Option[String])], isPrivate: Boolean = false, source: KeepSource = KeepSource("fake")): Seq[Keep] = {
    val bookmarks = edges.map {
      case (uri, user, optionalTitle) =>
        ...
        Keep(title = optionalTitle orElse uri.title, userId = user.id.get, uriId = uri.id.get, url = url.url, source = source, visibility = library.visibility, libraryId = Some(library.id.get), originalKeeperId = Some(user.id.get), messageThreadId = None)
    }
