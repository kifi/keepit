...
object LibraryMembershipFactory {
  ...
  class PartialLibraryMembership private[LibraryMembershipFactory] (membership: LibraryMembership) {
    ...
    def withLibraryCollaborator(libId: Id[Library], userId: Id[User]) = new PartialLibraryMembership(membership.copy(userId = userId, libraryId = libId, access = LibraryAccess.READ_WRITE))
    def withLibraryCollaborator(lib: Library, user: User) = new PartialLibraryMembership(membership.copy(userId = user.id.get, libraryId = lib.id.get, access = LibraryAccess.READ_WRITE))
    ...
    def withPriority(starred: LibraryPriority) = new PartialLibraryMembership(membership.copy(starred = starred))
