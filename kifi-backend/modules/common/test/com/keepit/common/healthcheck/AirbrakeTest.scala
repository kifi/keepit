  ...
  "AirbrakeTest" should {
    "format only error" in {
      withInjector(TestFortyTwoModule(), StandaloneTestActorSystemModule(), FakeHttpClientModule()) { implicit injector =>
        ...
        val error = AirbrakeError(new IllegalArgumentException("hi there", new IllegalStateException("its me")))
        val xml = formatter.format(error)
        println(xml)
        validate(xml)
        (xml \ "api-key").head === <api-key>fakeApiKey</api-key>
        (xml \ "error" \ "class").head === <class>java.lang.IllegalStateException</class>
        (xml \ "error" \ "message").head === <message>java.lang.IllegalStateException: its me</message>
