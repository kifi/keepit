...
trait UserLDAInterestsRepo extends DbRepo[UserLDAInterests] {
  ...
  def getAllUserTopicMean(version: ModelVersion[DenseLDA], minEvidence: Int)(implicit session: RSession): Seq[UserTopicMean]
}
...
class UserLDAInterestsRepoImpl @Inject() (
  ...
  def getAllUserTopicMean(version: ModelVersion[DenseLDA], minEvidence: Int)(implicit session: RSession): Seq[UserTopicMean] = {
    (for { r <- rows if (r.version === version && r.numOfEvidence > minEvidence && r.state === UserLDAInterestsStates.ACTIVE) } yield r.userTopicMean).list
  }
