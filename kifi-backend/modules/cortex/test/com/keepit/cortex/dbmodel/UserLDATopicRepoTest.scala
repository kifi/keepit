  ...
  "user lda topic repo" should {
    "work" in {
      withDb() { implicit injector =>
        val userTopicRepo = inject[UserLDATopicRepo]

        val topic = UserLDATopic(userId = Id[User](1), version = ModelVersion[DenseLDA](1), userTopicMean = Some(UserTopicMean(Array(0.5f, 0.25f, 0.15f, 0.1f))))
        db.readWrite { implicit s =>
          userTopicRepo.save(topic)
          userTopicRepo.getByUser(Id[User](1), ModelVersion[DenseLDA](1)).get.mean.toSeq === Seq(0.5f, 0.25f, 0.15f, 0.1f)
