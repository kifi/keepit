    ...
    "work" in {
      withDb(FakeShoeboxServiceModule()) { implicit injector =>
        ...
        db.readWrite { implicit s =>
          keepRepo.save(CortexKeep(keptAt = time, userId = Some(Id[User](1)), keepId = Id[Keep](1), uriId = Id[NormalizedURI](1L), state = State[CortexKeep]("active"), seq = SequenceNumber[CortexKeep](1L), source = KeepSource.keeper))
          keepRepo.save(CortexKeep(keptAt = time, userId = Some(Id[User](1)), keepId = Id[Keep](2), uriId = Id[NormalizedURI](2L), state = State[CortexKeep]("active"), seq = SequenceNumber[CortexKeep](2L), source = KeepSource.keeper))
