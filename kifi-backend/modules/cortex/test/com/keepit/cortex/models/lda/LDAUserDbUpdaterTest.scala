    ...
    "work" in {
      withDb() { implicit injector =>
        ...
        db.readWrite { implicit s =>
          ...
          val keeps = uris.map { uri => CortexKeep(keptAt = time, userId = Some(Id[User](1)), keepId = Id[Keep](1), uriId = uri.id.get, state = State[CortexKeep]("active"), seq = SequenceNumber[CortexKeep](uri.seq.value), source = KeepSource.Keeper) }
