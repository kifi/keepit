...
object TagCloudGenerator extends Logging {
  ...
  def generate(corpus: LibraryCorpus): SuggestedSearchTerms = {
    ...
    val topEntities = topKcounts(entityCnts, TOP_K * 2, smartCutoff = true)

    log.info("top keywords: " + topKeywords)
    log.info("top entities: " + topEntities)
...
object WordCountHelper {
  ...
  def topKcounts(wordCounts: WordCounts, topK: Int, smartCutoff: Boolean = false): WordCounts = {
    ...
    wordCounts.toArray.filter(_._2 > cutoff).sortBy(-_._2).take(topK).toMap
  }
}
