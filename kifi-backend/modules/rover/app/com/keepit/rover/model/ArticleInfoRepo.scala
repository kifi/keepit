...
trait ArticleInfoRepo extends Repo[RoverArticleInfo] with SeqNumberFunction[RoverArticleInfo] {
  ...
  def countRecentFetchesByDomain(lastFetchedWithin: Duration)(implicit session: RSession): Map[String, Int]
}
...
class ArticleInfoRepoImpl @Inject() (
  ...
  def countRecentFetchesByDomain(lastFetchedWithin: Duration)(implicit session: RSession): Map[String, Int] = {
    val recentlyFetchedRows = {
      ...
      for (r <- rows if r.lastFetchedAt > lastFetchedSomeTimeAgo) yield r
    }
