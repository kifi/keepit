...
import org.specs2.execute.{ Success, ResultExecution, Result, AsResult }
import org.specs2.mutable.Specification

import scala.concurrent.duration.Duration
import scala.concurrent.{ Await, Future }

class DefaultArticleFetcherTest extends Specification with RoverTestInjector {

  def mkRequest(file: String): ArticleFetchRequest[DefaultArticle] = {
    val fileLocation = s"test/com/keepit/rover/article/fetcher/fixtures/$file"
    ArticleFetchRequest(DefaultArticle, fileLocation)
  }

  def fetch(file: String)(implicit articleFetcher: ArticleFetcher[DefaultArticle]): DefaultArticle =
  ...
  withInjector(FileHttpFetcherModule()) { implicit injector =>
    ...
    implicit val articleFetcher = inject[DefaultArticleFetcher]
