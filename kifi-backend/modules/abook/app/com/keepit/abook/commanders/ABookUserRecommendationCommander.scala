  ...
  private def generateFutureFriendRecommendations(userId: Id[User]): Future[Option[Stream[(Id[User], Double)]]] = {
    ...
    futureRelatedUsers.flatMap {
      ...
      case Some(relatedUsers) => for {
        ...
        Some(relatedUsers.related.toStream.collect { case (friendId, rawScore) if !irrelevantRecommendations.contains(friendId) => friendId -> (rawScore / relatedUsers.normalizingConstant) })
      }
    }
  }
      ...
      relatedEntities.map(_.emailAccounts) match {
        ...
        case Some(relatedEmailAccounts) => {
          ...
          for {
            ...
            generateEmailInviteRecommendations(relatedEmailAccounts, rejectedEmailInviteRecommendations, allContacts, normalizedUserNames, existingInvites)
          }
        }
      }
    }
        ...
        case Some(relatedOrganizations) => {
          ...
          relatedOrganizations.related.toStream.collect {
            case (orgId, score) if !rejectedOrganizationRecommendations.contains(orgId) =>
              OrganizationUserMayKnow(orgId, score / relatedOrganizations.normalizingConstant)
          }
        }
      }
    }
  ...
  private def generateSocialInviteRecommendations(
    ...
    val recommendations = relatedSocialUsers.related.toStream.collect {
      case (socialUserId, score) if isRelevant(socialUserId) =>
        ...
        UserInviteRecommendation(friend.networkType, Right(friend.socialId), Some(friend.fullName), friend.getPictureUrl(80, 80), lastInvitedAt, score / relatedSocialUsers.normalizingConstant)
    }
  ...
  private def generateEmailInviteRecommendations(
    ...
    val recommendations = relatedEmailAccounts.related.toStream.collect {
      case (emailAccountId, score) if isRelevant(emailAccountId) =>
        ...
        UserInviteRecommendation(SocialNetworks.EMAIL, Left(preferredContact.email), validName, None, lastInvitedAt, score / relatedEmailAccounts.normalizingConstant)
    }
