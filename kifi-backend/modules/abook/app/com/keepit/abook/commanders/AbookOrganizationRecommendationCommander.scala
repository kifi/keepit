...
class AbookOrganizationRecommendationCommander @Inject() (
  ...
  private def generateFutureRecommendations(orgId: Id[Organization], viewerIdOpt: Option[Id[User]]): Future[Stream[OrganizationInviteRecommendation]] = {
    ...
    for {
      ...
      relatedEntitiesOpt <- fRelatedEntities
    } yield {
      relatedEntitiesOpt.map { relatedEntities =>
        val totalSteps = relatedEntities.emailAccounts.totalSteps + relatedEntities.users.totalSteps
        CollectionHelpers.interleaveBy(
          emailRecommendations.map(reco => reco.copy(score = reco.score * relatedEntities.emailAccounts.totalSteps / totalSteps)),
          userRecommendations.map(reco => reco.copy(score = reco.score * relatedEntities.users.totalSteps / totalSteps))
        )(_.score)
  ...
  private def generateFutureUserRecommendations(orgId: Id[Organization], fRelatedEntities: Future[Option[SociallyRelatedEntitiesForOrg]]): Future[Option[Stream[OrganizationInviteRecommendation]]] = {
    ...
    fRelatedUsers.flatMap {
      ...
      case Some(relatedUsers) => for {
        ...
        val recommendations = relatedUsers.related.map { case (userId, score) => OrganizationInviteRecommendation(Left(userId), None, score / relatedUsers.totalSteps) }
  ...
  private def generateEmailInviteRecommendations(
    ...
    val recommendations = relatedEmailAccounts.related.toStream.collect {
      case (emailAccountId, score) if isRelevant(emailAccountId) =>
        ...
        OrganizationInviteRecommendation(Right(preferredContact.email), validName, score / relatedEmailAccounts.totalSteps)
    }
