...
class OrganizationMemberRecommendationRepoImpl @Inject() (
  ...
  class MemberRecommendationTable(tag: Tag) extends RepoTable[OrganizationMemberRecommendation](db, tag, "member_recommendation") {
    ...
    def * = (id.?, createdAt, updatedAt, organizationId, memberId, irrelevant) <> ((OrganizationMemberRecommendation.apply _).tupled, OrganizationMemberRecommendation.unapply _)
  }
  ...
  private def internMemberRecommendation(organizationId: Id[Organization], memberId: Id[User], irrelevant: Boolean)(implicit session: RWSession): OrganizationMemberRecommendation = {
    compiledGetByOrgAndMember(organizationId, memberId).firstOption match {
      case None => save(OrganizationMemberRecommendation(organizationId = organizationId, memberId = memberId, irrelevant = irrelevant))
