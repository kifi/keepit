...
class ContactsUpdaterActorPlugin @Inject() (
    ...
    nrOfInstances: Int) extends ContactsUpdaterPlugin with Logging {
...
class ContactsUpdaterActor @Inject() (
    ...
    contactsUpdater: ContactsUpdater) extends FortyTwoActor(airbrake) with Logging {
        ...
        case Some(abookRawInfo) =>
          {
            ...
            abookRawInfo.contacts.value.grouped(abookUploadConf.batchSize).foreach { g =>
              ...
              g.foreach { contact =>
                ...
                val emails = (contact \ "emails").asOpt[Seq[JsValue]].getOrElse(Seq.empty[JsValue]).foldLeft(Seq[EmailAddress]()) {
