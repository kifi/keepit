...
class SlackSearchController @Inject() (
  ...
  def search(
    ...
    command: String,
    text: String,
    responseUrl: String) = MaybeUserAction.async(parse.tolerantJson) { request =>
    if (command == "/kifi" && token == KifiSlackApp.SLACK_COMMAND_TOKEN.value) {
      shoeboxClient.getIntegrationsBySlackChannel(SlackTeamId(teamId), SlackChannelId(channelId)).flatMap {
        case integrations =>
          val futureResponse = {
            ...
            val futureTextAndAttachments: Future[(String, Seq[SlackAttachment])] = integrations.filter(_.on).maxByOpt(_.lastMessageTimestamp) match {
              case Some(integration) if integration.lastMessageTimestamp.isDefined =>
                ...
                val sourceScope = SourceScope(KeepFields.Source.apply(SlackChannelId(channelId)))
                ...
                uriSearchCommander.searchUris(userId, acceptLangs, experiments, text, Future.successful(searchContext), maxUris, None, None, None).flatMap { uriSearchResult =>
                  ...
                  for {
                    ...
                    val text = {
                      ...
                      else s"Here are the most relevant links we found in $channelName:"
                    }
                    (text, attachments)
                  }
                }
          ...
          futureResponse.map { r =>
            ...
            httpClient.postFuture(DirectUrl(responseUrl), result)
