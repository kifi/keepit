        ...
        val attachments = relevantHits.map { hit =>
          ...
          val pretext = {
            val attribution = keepId.flatMap(sourceAttributions.get).map {
              ...
              case SlackAttribution(message) => Elements("via", "@" + message.user.name.value, "in", "#" + message.channel.name.value, "·", message.timestamp.toDateTime --> LinkElement(message.permalink))
            }
