...
class UserSearcher(searcher: Searcher) {
  def search(query: Query, maxHit: Int = 10, idFilter: Set[Long] = Set.empty[Long]): UserSearchResult = {
    ...
    searcher.doSearch(query){ (scorer, reader) =>
      ...
      while(doc != NO_MORE_DOCS){
        ...
        if (id >= 0 && !idFilter.contains(id)){
          ...
          pq.insertWithOverflow(UserHit(userId, user))
        }
        doc = scorer.nextDoc()
      }
    }
  ...
  def searchWithFilter(query: Query, maxHit: Int = 10, searchFilter: UserSearchFilter): UserSearchResult = {
  ...
  def searchPagingWithFilter(query: Query, searchFilter: UserSearchFilter, pageNum: Int, pageSize: Int): UserSearchResult = {
