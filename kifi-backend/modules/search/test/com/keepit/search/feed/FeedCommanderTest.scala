...
class FeedCommanderTest extends Specification with SearchApplicationInjector with SearchTestHelper {
  ...
  def setup(client: FakeShoeboxServiceClientImpl) = {
    ...
    val bms = client.saveBookmarks(
      Bookmark(createdAt = t0, uriId = uris(0).id.get, url = uris(0).url, userId = users(1).id.get, source = KeepSource.keeper, isPrivate = false),

      Bookmark(createdAt = t0.plusMinutes(1), uriId = uris(1).id.get, url = uris(1).url, userId = users(1).id.get, source = KeepSource.keeper, isPrivate = false),
      Bookmark(createdAt = t0.plusMinutes(2), uriId = uris(1).id.get, url = uris(1).url, userId = users(2).id.get, source = KeepSource.keeper, isPrivate = false),
      Bookmark(createdAt = t0.plusMinutes(3), uriId = uris(1).id.get, url = uris(1).url, userId = users(3).id.get, source = KeepSource.keeper, isPrivate = false),

      Bookmark(createdAt = t0.plusMinutes(4), uriId = uris(2).id.get, url = uris(2).url, userId = users(0).id.get, source = KeepSource.keeper, isPrivate = false),
      Bookmark(createdAt = t0.plusMinutes(5), uriId = uris(2).id.get, url = uris(2).url, userId = users(2).id.get, source = KeepSource.keeper, isPrivate = false),

      Bookmark(createdAt = t0.plusMinutes(6), uriId = uris(3).id.get, url = uris(3).url, userId = users(1).id.get, source = KeepSource.keeper, isPrivate = false),

      Bookmark(createdAt = t0.plusMinutes(7), uriId = uris(4).id.get, url = uris(4).url, userId = users(2).id.get, source = KeepSource.keeper, isPrivate = false),
      Bookmark(createdAt = t0.plusMinutes(8), uriId = uris(4).id.get, url = uris(4).url, userId = users(3).id.get, source = KeepSource.keeper, isPrivate = false),

      Bookmark(createdAt = t0.plusMinutes(9), uriId = uris(5).id.get, url = uris(5).url, userId = users(2).id.get, source = KeepSource.keeper, isPrivate = true)
    )
