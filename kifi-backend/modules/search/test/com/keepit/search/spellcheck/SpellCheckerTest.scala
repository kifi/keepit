...
class SpellCheckerTest extends Specification {
  ...
  val articles = Seq("abc abc abc", "abc def", "abc xyz")
  ...
  "spell correcter" should {
    "build dict from exisiting Lucene index" in {
      ...
      val spellIndexer = SpellIndexer(spellIndexDir, articleIndexDir, SpellIndexerConfig(1f))
      ...
      corrector.getAlternativeQuery("abcd") === "abc"
    }
  }
}
