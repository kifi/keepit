  ...
  "ShardedArticleIndexer" should {
    "create index shards" in {
      withInjector(helperModules: _*) { implicit injector =>
        ...
        val (uriGraph, collectionGraph, indexer, userGraphIndexer, userGraphsSearcherFactory, mainSearcherFactory, _, _, _) = initIndexes(store)
    ...
    "correctly reindex" in {
      withInjector(helperModules: _*) { implicit injector =>
        ...
        val store = mkStore(uris)
        val (uriGraph, collectionGraph, indexer, userGraphIndexer, _, mainSearcherFactory, _, _, _) = initIndexes(store)
        indexer.isInstanceOf[ShardedArticleIndexer] === true
