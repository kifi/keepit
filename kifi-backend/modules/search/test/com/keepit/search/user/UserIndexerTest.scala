...
class UserIndexerTest extends Specification with ApplicationInjector {
  private def setup(implicit client: FakeShoeboxServiceClientImpl) = {
    ...
    val emails = (0 until 4).map{ i =>
      UserEmailAddress(userId = usersWithId(i).id.get, address = s"user${i}@42go.com")
    } ++ Seq(UserEmailAddress(userId = usersWithId(4).id.get, address = "woody@fox.com"),
     UserEmailAddress(userId = usersWithId(4).id.get, address = "Woody.Allen@GMAIL.com"))
  ...
  "UserIndxer" should {
    "persist sequence number" in {
      running(new TestApplication(FakeShoeboxServiceModule())){
        ...
        client.saveEmails(UserEmailAddress(userId = newUsers(0).id.get, address = "abc@xyz.com"))
        indexer.update()
