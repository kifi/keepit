  ...
  "HelpRankCommander" should {
    ...
    "track discoveries & rekeeps" in {
      withDb(modules: _*) { implicit injector =>
        ...
        val u1 = User(id = Some(Id[User](1)), createdAt = ts, updatedAt = ts, firstName = "Shanee", lastName = "Smith", username = Username("test"), normalizedUsername = "test", pictureName = Some("0"))
        val u2 = User(id = Some(Id[User](2)), createdAt = ts, updatedAt = ts, firstName = "Foo", lastName = "Bar", username = Username("test"), normalizedUsername = "test", pictureName = Some("0"))
        val u3 = User(id = Some(Id[User](3)), createdAt = ts, updatedAt = ts, firstName = "Ping", lastName = "Pong", username = Username("test"), normalizedUsername = "test", pictureName = Some("0"))
        val u4 = User(id = Some(Id[User](4)), createdAt = ts, updatedAt = ts, firstName = "Ro", lastName = "Bot", username = Username("test"), normalizedUsername = "test", pictureName = Some("0"))
        ...
        val kc0 = db.readWrite { implicit rw =>
          ...
          // u2 -> 42 (u1) [discovery]
          kifiHitCache.set(KifiHitKey(u2.id.get, k1(FORTYTWO).uriId), SanitizedKifiHit(kc0.hitUUID, origin, k1(FORTYTWO).url, kc0.uriId, KifiHitContext(false, false, 0, Seq(u1.externalId), Seq.empty, None, 0, 0)))
          kc0
        }
        ...
        val (kc1, kc2, kc3) = db.readWrite { implicit rw =>
          ...
          kifiHitCache.set(KifiHitKey(u3.id.get, k1(KIFI).uriId), SanitizedKifiHit(kc1.hitUUID, origin, k1(KIFI).url, kc1.uriId, KifiHitContext(false, false, 0, Seq(u1.externalId, u2.externalId), Seq.empty, None, 0, 0)))
          ...
          kifiHitCache.set(KifiHitKey(u3.id.get, k2(BING).uriId), SanitizedKifiHit(kc3.hitUUID, origin, k2(BING).url, kc3.uriId, KifiHitContext(false, false, 0, Seq(u2.externalId), Seq.empty, None, 0, 0)))
          (kc1, kc2, kc3)
        }
        ...
        val kc4 = db.readWrite { implicit rw =>
          ...
          // u4 -> kifi (u3) [rekeep]
          kifiHitCache.set(KifiHitKey(u4.id.get, k3(KIFI).uriId), SanitizedKifiHit(kc4.hitUUID, origin, k3(KIFI).url, kc4.uriId, KifiHitContext(false, false, 0, Seq(u3.externalId), Seq.empty, None, 0, 0)))
          kc4
        }
