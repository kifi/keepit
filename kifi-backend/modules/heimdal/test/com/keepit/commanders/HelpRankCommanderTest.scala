...
class HelpRankCommanderTest extends Specification with HeimdalTestInjector with NoTimeConversions {
  ...
  def mkKeep(userId: Id[User], ts: DateTime = currentDateTime, idOpt: Option[Id[Keep]] = None, libraryId: Option[Id[Library]] = None)(uri: NormalizedURI): Keep = {
    ...
    KeepFactory.keep().withId(id).withCreatedAt(ts).withUri(uri).withLibraryId((Id(1), LibraryVisibility.PUBLISHED, None)).withUser(userId).get
  }
  ...
  "HelpRankCommander" should {
    ...
    "track discoveries & rekeeps" in {
      withDb(modules: _*) { implicit injector =>
        ...
        val u1 = UserFactory.user().withId(1).withCreatedAt(ts).withUpdatedAt(ts).withName("Shanee", "Smith").withUsername("test").get
        ...
        val u4 = UserFactory.user().withId(4).withCreatedAt(ts).withUpdatedAt(ts).withName("Ro", "Bot").withUsername("test").get
        ...
        db.readWrite { implicit rw =>
          (keeps1 ++ keeps2 ++ keeps3 ++ keeps4).groupBy(k => (k.userId.get, k.uriId)).keySet.foreach {
            case (userId, uriId) =>
