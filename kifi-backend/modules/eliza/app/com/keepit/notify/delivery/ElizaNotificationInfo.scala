...
class ElizaNotificationInfo @Inject() (
  ...
  def mkJson(notif: Notification, items: Set[NotificationItem], info: NotificationInfo): Future[JsValue] = {
    val maxByTime = items.maxBy(_.event.time)
    val maxTime = maxByTime.event.time
    resolveImage(info.image).map { image =>
      Json.obj(
  ...
  def resolveImage(image: NotificationImage): Future[String] = image match {
    ...
    case PublicImage(url) => Future.successful(url)
  }
