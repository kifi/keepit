      ...
      case JsString(keepIdStr) +: (data: JsValue) +: _ =>
        ...
        if (users.nonEmpty || emailContacts.nonEmpty || orgs.nonEmpty) {
          ...
          Keep.decodePublicIdStr(keepIdStr).foreach { keepId =>
            ...
            } discussionCommander.editParticipantsOnKeep(keepId, socket.userId, userIds, emailContacts, orgIds, KeepEventSourceKind.fromStr(socket.userAgent), updateShoebox = true)
          }
        }
    },
