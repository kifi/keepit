...
class SharedWsMessagingController @Inject() (
  ...
  protected def websocketHandlers(socket: SocketInfo) = Map[String, Seq[JsValue] => Unit](
    ...
    "add_participants_to_thread" -> { case JsString(threadId) +: (data: JsValue) +: _ =>
      ...
      messagingCommander.addUsersToThread(socket.userId, ExternalId[MessageThread](threadId), users, source)
      messagingCommander.addEmailNonUsersToThread(socket.userId, ExternalId[MessageThread](threadId), emailAddresses, source)
    },
