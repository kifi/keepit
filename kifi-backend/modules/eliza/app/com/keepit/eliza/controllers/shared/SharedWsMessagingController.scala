...
import com.keepit.eliza.commanders.{ NotificationJson, MessageFetchingCommander, NotificationCommander, MessagingCommander }
...
class SharedWsMessagingController @Inject() (
  ...
  private def onFailSendErr(socket: SocketInfo, requestId: BigDecimal, fut: Future[Any]): Future[Unit] = {
    fut recover {
      case _ => socket.channel.push(Json.arr("server_error", requestId.toLong))
    } map { _ => () }
  }

  protected def websocketHandlers(socket: SocketInfo) = Map[String, PartialFunction[Seq[JsValue], Future[Unit]]](
    "ping" -> {
      case _ =>
        Future {
      ...
      case JsString(threadId) +: _ =>
        ...
        basicMessageCommander.getThreadMessagesWithBasicUser(socket.userId, ExternalId[MessageThread](threadId)) map {
          case (thread, msgs) =>
            ...
            socket.channel.push(Json.arr("thread", Json.obj("id" -> threadId, "uri" -> url, "messages" -> msgs.reverse)))
        }
    },
    ...
    "get_one_thread" -> {
      case JsNumber(requestId) +: JsString(threadId) +: _ =>
        notificationCommander.getSendableNotification(socket.userId, ExternalId[MessageThread](threadId), needsPageImages(socket)) andThen {
          case Success(json) =>
    ...
    "get_latest_threads" -> {
      case JsNumber(requestId) +: JsNumber(howMany) +: _ =>
        onFailSendErr(socket, requestId, notificationCommander.getLatestSendableNotifications(socket.userId, howMany.toInt, needsPageImages(socket)) map { notices =>
    ...
    "get_threads_before" -> {
      case JsNumber(requestId) +: JsNumber(howMany) +: JsString(time) +: _ =>
        onFailSendErr(socket, requestId,
          notificationCommander.getSendableNotificationsBefore(socket.userId, parseStandardTime(time), howMany.toInt, needsPageImages(socket)) map {
            notices => socket.channel.push(Json.arr(requestId.toLong, notices.map(_.obj)))
          })
    },
    "get_unread_threads" -> {
      case JsNumber(requestId) +: JsNumber(howMany) +: _ =>
        onFailSendErr(socket, requestId,
          notificationCommander.getLatestUnreadSendableNotifications(socket.userId, howMany.toInt, needsPageImages(socket)) map {
    ...
    "get_unread_threads_before" -> {
      case JsNumber(requestId) +: JsNumber(howMany) +: JsString(time) +: _ =>
        onFailSendErr(socket, requestId,
          notificationCommander.getUnreadSendableNotificationsBefore(socket.userId, parseStandardTime(time), howMany.toInt, needsPageImages(socket)) map {
            notices => socket.channel.push(Json.arr(requestId.toLong, notices.map(_.obj)))
          })
    },
    "get_sent_threads" -> {
      case JsNumber(requestId) +: JsNumber(howMany) +: _ =>
        onFailSendErr(socket, requestId,
          notificationCommander.getLatestSentSendableNotifications(socket.userId, howMany.toInt, needsPageImages(socket)) map {
            notices => socket.channel.push(Json.arr(requestId.toLong, notices.map(_.obj)))
          })
    },
    "get_sent_threads_before" -> {
      case JsNumber(requestId) +: JsNumber(howMany) +: JsString(time) +: _ =>
        onFailSendErr(socket, requestId,
          notificationCommander.getSentSendableNotificationsBefore(socket.userId, parseStandardTime(time), howMany.toInt, needsPageImages(socket)) map {
            notices => socket.channel.push(Json.arr(requestId.toLong, notices.map(_.obj)))
          })
    },
    "get_page_threads" -> {
      case JsNumber(requestId) +: JsString(url) +: JsNumber(howMany) +: _ =>
        onFailSendErr(socket, requestId,
          notificationCommander.getLatestSendableNotificationsForPage(socket.userId, url, howMany.toInt, needsPageImages(socket)) map {
            case (nUriStr, notices, numTotal, numUnreadUnmuted) => socket.channel.push(Json.arr(requestId.toLong, nUriStr, notices.map(_.obj), numTotal, numUnreadUnmuted))
          })
    },
    "get_page_threads_before" -> {
      case JsNumber(requestId) +: JsString(url) +: JsNumber(howMany) +: JsString(time) +: _ =>
        onFailSendErr(socket, requestId,
          notificationCommander.getSendableNotificationsForPageBefore(socket.userId, url, parseStandardTime(time), howMany.toInt, needsPageImages(socket)) map {
            notices => socket.channel.push(Json.arr(requestId.toLong, notices.map(_.obj)))
          })
    },
