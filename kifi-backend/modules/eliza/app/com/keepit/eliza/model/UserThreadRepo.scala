...
trait UserThreadRepo extends Repo[UserThread] with RepoWithDelete[UserThread] {
  ...
  def checkUrisChatted(userId: Id[User], uriIds: Seq[Id[NormalizedURI]])(implicit session: RSession): Seq[Boolean]
...
class UserThreadRepoImpl @Inject() (
  ...
  def checkUrisChatted(userId: Id[User], uriIds: Seq[Id[NormalizedURI]])(implicit session: RSession): Seq[Boolean] = {
    ...
    val uris = (for (row <- rows if row.user === userId) yield row.uriId).list
    uris.map(uri => uriSet.contains(uri)).toSeq
  }
