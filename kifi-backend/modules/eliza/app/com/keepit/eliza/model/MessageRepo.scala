...
trait MessageRepo extends Repo[ElizaMessage] with SeqNumberFunction[ElizaMessage] {
  ...
  //migration
  def pageSystemMessagesInclusive(fromId: Id[ElizaMessage], pageSize: Int)(implicit session: RSession): Seq[ElizaMessage]
}
...
class MessageRepoImpl @Inject() (
  ...
  def pageSystemMessagesInclusive(fromId: Id[ElizaMessage], pageSize: Int)(implicit session: RSession): Seq[ElizaMessage] = {
    val fromTime = activeRows.filter(_.id === fromId).map(_.createdAt).list.head
    activeRows.filter(msg => msg.auxData.isDefined && msg.createdAt >= fromTime).sortBy(_.id asc).take(pageSize).list
  }
