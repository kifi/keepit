...
class MessageThreadRepoImpl @Inject() (
  ...
  class MessageThreadTable(tag: Tag) extends RepoTable[MessageThread](db, tag, "message_thread") with ExternalIdColumn[MessageThread] {
    ...
    def * = (id.?, createdAt, updatedAt, externalId, uriId.?, url.?, nUrl.?, pageTitle.?, participants.?, participantsHash.?, keepId, asyncStatus) <> ((MessageThread.apply _).tupled, MessageThread.unapply _)
  }
