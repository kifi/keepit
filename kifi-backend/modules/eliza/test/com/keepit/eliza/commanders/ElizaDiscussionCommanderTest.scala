    ...
    "serve up discussions by keep" in {
      "do the right thing for keeps with and without messages" in {
        withDb(modules: _*) { implicit injector =>
          ...
          val ans = Await.result(discussionCommander.getCrossServiceDiscussionsForKeeps(Set(keep, keepNoMessages, keepNoThread), fromTime = None, maxMessagesShown = 5), Duration.Inf)
      ...
      "dont' blow up on system messages" in {
        withDb(modules: _*) { implicit injector =>
          ...
          val ans = Await.result(discussionCommander.getCrossServiceDiscussionsForKeeps(Set(keep), fromTime = None, maxMessagesShown = 5), Duration.Inf)
    ...
    "add messages to discussions" in {
      "add new commenters as they chime in" in {
        withDb(modules: _*) { implicit injector =>
          ...
          val ans = Await.result(discussionCommander.getCrossServiceDiscussionsForKeeps(Set(keep), fromTime = None, maxMessagesShown = 5), Duration.Inf).get(keep)
