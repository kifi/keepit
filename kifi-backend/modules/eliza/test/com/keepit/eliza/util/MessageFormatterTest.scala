...
class MessageFormatterTest extends Specification {
  ...
  "MessageFormatter.parseMessage" should {
    ...
    "parse message text that looks like a look here link but is not #1" in {
      val msg = """[\](x-kifi-sel:r|body|p.a|0:1|p.b|0:4|x)"""
      MessageFormatter.parseMessageSegments(msg) === Seq(TextSegment(msg))
    }

    "parse message text that looks like a look here link but is not #2" in {
      val msg = """[\\\](x-kifi-sel:r|body|p.a|0:1|p.b|0:4|x)"""
      MessageFormatter.parseMessageSegments(msg) === Seq(TextSegment(msg))
    }

    "parse message text that looks like a look here link but is not #3" in {
      val msg = """[\\\\\](x-kifi-sel:r|body|p.a|0:1|p.b|0:4|x)"""
      MessageFormatter.parseMessageSegments(msg) === Seq(TextSegment(msg))
    }

    "parse message text that looks like a look here link but is not #4" in {
      val msg = """[\\\\\\\](x-kifi-sel:r|body|p.a|0:1|p.b|0:4|x)"""
      MessageFormatter.parseMessageSegments(msg) === Seq(TextSegment(msg))
    }

    "parse message text that looks like a look here link but is not #5" in {
      val msg = """[look](x-kifi-sel:r|body|p.a|0:1|p.b|0:4|\)"""
      MessageFormatter.parseMessageSegments(msg) === Seq(TextSegment(msg))
    }

    "parse message text that looks like a look here link but is not #6" in {
      val msg = """[look](x-kifi-sel:r|body|p.a|0:1|p.b|0:4|\\\)"""
      MessageFormatter.parseMessageSegments(msg) === Seq(TextSegment(msg))
    }

    "parse message text that looks like a look here link but is not #7" in {
      val msg = """[look](x-kifi-sel:r|body|p.a|0:1|p.b|0:4|\\\\\)"""
      MessageFormatter.parseMessageSegments(msg) === Seq(TextSegment(msg))
    }

    "parse message text that looks like a look here link but is not #8" in {
      val msg = """[look](x-kifi-sel:r|body|p.a|0:1|p.b|0:4|\\\\\\\)"""
      MessageFormatter.parseMessageSegments(msg) === Seq(TextSegment(msg))
    }
  }
