  ...
  "Messaging Contoller" should {
    ...
    "send correctly" in {
      withDb(modules: _*) { implicit injector =>
        ...
        Await.result(notificationCommander.getLatestSendableNotifications(user1, 20), Duration(4, "seconds")).length === 1
    ...
    "merge and notify correctly" in {
      withDb(modules: _*) { implicit injector =>
        ...
        val notifications: Seq[JsObject] = Await.result(notificationCommander.getLatestUnreadSendableNotifications(user3, 20), Duration(4, "seconds"))._1.map(_.obj)
    ...
    "add participants correctly" in {
      withDb(modules: _*) { implicit injector =>
        ...
        Await.result(notificationCommander.getLatestSendableNotifications(user2, 1), Duration(4, "seconds")).length === 1
        Await.result(notificationCommander.getLatestSendableNotifications(user3, 1), Duration(4, "seconds")).length === 0
        ...
        Await.result(notificationCommander.getLatestSendableNotifications(user3, 1), Duration(4, "seconds")).length === 1
      }
    }
