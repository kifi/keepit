...
class FakeSecureSocialUserPlugin extends SecureSocialUserPlugin {
  ...
  override def find(id: IdentityId): Option[SocialUser] = Option(id).filter(_ == FakeSecureSocial.FAKE_IDENTITY_ID).map { _ =>
    FakeSecureSocial.FAKE_SOCIAL_USER
  }
  ...
  override def save(identity: Identity): SocialUser = FakeSecureSocial.FAKE_SOCIAL_USER
