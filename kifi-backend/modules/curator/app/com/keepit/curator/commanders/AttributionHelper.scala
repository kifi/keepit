    ...
    seeds.headOption.map { _.userId } match {
      ...
      case Some(userId) =>
        ...
        if (uriId2Idx.size == 0) {
          ...
          search.augment(Some(userId), showPublishedLibraries = false, maxKeepersShown = 20, maxLibrariesShown = 15, maxTagsShown = 0, items = uriIds.map(AugmentableItem(_))).map { infos =>
            timer.stopAndReport()
