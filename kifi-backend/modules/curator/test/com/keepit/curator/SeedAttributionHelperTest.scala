...
class SeedAttributionHelperTest extends Specification with CuratorTestInjector {
  ...
  val fakeSearch = new FakeSearchServiceClient() {
    ...
    override def augment(userId: Option[Id[User]], maxKeepersShown: Int, maxLibrariesShown: Int, maxTagsShown: Int, items: Seq[AugmentableItem], showPublishedLibraries: Boolean = false): Future[Seq[LimitedAugmentationInfo]] = {
