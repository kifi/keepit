...
serviceNameLo="eliza"                                  # service name with the first letter in lowercase
serviceName="Eliza"                                    # service name
...
javaArgs="$yourkit $newrelic $jmxArgs $javaGc -Ddb.$serviceNameLo-slave.password=keepmydatasecure -Ddb.$serviceNameLo.password=keepmydatasecure -Dhttp.port=9000 -Xms512M -Xmx3072m -server -Dconfig.resource=prod/$serviceNameLo.conf -Duser.dir=$applDir/ -Dlogger.resource=prod/logger.prod.xml -cp $classpath play.core.server.NettyServer $applDir"
